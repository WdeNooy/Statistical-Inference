...
output$scatterplot <- renderPlot({
  ...
  ggplot(df, aes(x = smokestatus, y = attitude, colour = smokestatus)) + 
    ...
    geom_text(inherit.aes=FALSE,x = 1.5,y=regfunc(1.5,int = coef(mod1)[1],bet1 = coef(mod1)[2]) + 0.8,
              label = paste0("b = ", rprint(ifelse(input$selector == "Former smoker" & meaningroup$means[2] > meaningroup$means[1], -1 * coef(mod1)[2], coef(mod1)[2])), "\n", pprint(pval1)),
              colour = "grey", 
              alpha = ifelse(input$selector == "Smoker", 0, 1)) +
    #coefficient for non-smoker - smoker line: opposite sign if smoker is ref cat and has higher mean
    geom_text(inherit.aes=FALSE,x = 2.0,y=regfunc(2.0,int = coef(mod1)[1],bet1 = coef(mod1)[3]/2) - 0.8,
              label = paste0("b = ", rprint(ifelse(input$selector == "Smoker" & meaningroup$means[3] > meaningroup$means[1], -1 * coef(mod1)[3], coef(mod1)[3])), "\n", pprint(pval2)),
              colour = "black", 
              alpha = ifelse(input$selector == "Former smoker", 0, 1)) +
    #coefficient for smoker - former smoker line: opposite sign if smoker is ref cat and has higher mean 
    geom_text(inherit.aes=FALSE,x = 2.5,y=regfunc(2.5,int = coef(mod2)[1]- coef(mod2)[3],bet1 = coef(mod2)[3]) + 0.8,
              label = paste0("b = ", rprint(ifelse(input$selector == "Smoker" & meaningroup$means[3] > meaningroup$means[2], -1 * coef(mod2)[3], coef(mod2)[3])), "\n", pprint(pval3)), 
              colour = ifelse(input$selector == "Former smoker","black","grey"),
              alpha = ifelse(input$selector == "Non-smoker", 0, 1)) +
