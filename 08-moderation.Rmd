...
# TBD: add videos on executing regression, inspecting residuals, creating dummy variables, adding a regression line to a scattergram for a simple regression (Chart Editor: Add fit line at total) and one predictor in multiple regression (Chart Editor: reference line from equation), interpreting resuls. (Results reporting is discussed at the end of this chapter.)
# Chart Editor of zresid * zpred plot: add reference line at 0 and perhaps at +2 and -2 to inspect shape of residual distribution.
```
...
2. Visualize the moderated effects of exposure on attitude (Exercise 1). Create a scatterplot with three regression lines. Colour the regression lines and the dots (respondents) according to their smoking status category. Interpret the moderation of the exposure effect by smoking status.
...
4. Repeat the analyses of Exercises 1 thru 3 but use smoking status with three categories (_status3_).
...
Remember that the interaction variable is the product of the predictor and moderator (Section \@ref(interaction-variable)). If any or both of these are mean-centered, you should multiply the mean-centered variable(s) toz create the interaction variable.
...
# Colour lines for cont moderator by shades of blue, add
# annotations to explain the level of the moderator.

# ; check common support: group moderator in 3 groups (terciles) and create (panelled) histograms for the predictor scores in each moderator group ; Transform > Visual Binning
#
...
6. Is the effect of parental supervison moderated by sex? Use the data of Exercise 5 to answer this question. You may omit the age predictor from the model. Again, illustrate your answer with a moderation plot.
