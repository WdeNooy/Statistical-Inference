...
> Key concepts: regression equation, dummy variables, normally distributed residuals, linearity, homoscedasticity, independent observations, statistical diagram, interaction variable, covariate, common support, simple slope, conditional effect, mean-centering.
...
As an independent variable in the model, an interaction variable has a confidence interval and a p value. The confidence interval tells us the plausible values for the size of the interaction effect in the population. The p value tests the null hypothesis that there is no interaction effect at all in the population.

To interpret the interaction effect, we must determine the size of the effect of the predictor on the dependent variable for several interesting values of the moderator. If the moderator is categorical, we want to know the effect (simple slope) within each category of the moderator. For example, the effect of campaign exposure on smoking attitude for smokers and the effect for non-smokers. 

The moderator can also be a continuous variable, for example, the time a person spends with people who smoke. In this case, we may look at the effect for the mean value of the moderator (moderate score level) and one standard deviation below (low level) or above (high level) the mean of the moderator.

An interaction effect in a regression model closely resembles an interaction effect in analysis of variance. The effect of a single predictor in a regression model, however, is not a main effect as in analysis of variance. It is a conditional effect, namely the effect for one particular value of the moderator (context). To understand this, we must pay close attention to the regression equation.
...
Figure \@ref(fig:concept-smoke) summarizes some hypothesized causes of the attitude towards  smoking. Attitude towards smoking is measured as a scale, so it is a numerical variable. In linear regression, the dependent variable ($y$) must be numerical and in principle continuous. There are regression models for other types of dependent variables, for instance, logistic regression for a dichotomous (0/1) dependent variable and Poisson regression for a count dependent variable, but we will not discuss them.
...
```{r regression-continuous, fig.cap="Predicting attitude towards smoking from exposure to an anti-smoking campaign. The orange dot represents the predicted attitude for a selected value of exposure.", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
...
knitr::include_app("http://82.196.4.233:3838/apps/regression-continuous/", height="405px")
```
...
```{block2, type='rmdquestion'}
2. What does the vertical orange line mean in Figure \@ref(fig:regression-continuous) if exposure is set to 1? [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.2)
```
...
The constant is the predicted attitude if a person scores zero on all independent variables. To see this, plug in zero for the predictor in the equation (Eq. \@ref(eq:regsmokedummy)) and remember that zero times something yields zero. This reduces the equation to the constant.
...
```{r regression-dichotomy, eval=TRUE, echo=FALSE, fig.cap="What is the difference in attitude between non-smokers and smokers?", screenshot.opts = list(delay = 5), dev="png", out.width="775px"}
...
```{block2, type='rmdquestion'}
5. Change the group means to detect the relation between group means and the unstandardized regression coefficient ($b$). How can we calculate the unstandardized regression coefficient ($b$) from the group averages? Show that your answer is correct by changing average attitude towards smoking with the slider for smokers. [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.5)
```

Apart from numerical independent variables, we can use dichotomous independent variables, that is, variables with only two values, which are preferably coded as 0 and 1 (_dummy coding_). The interpretation of the effect of a dichotomous independent variable in a regression model is quite different from the interpretation of a numerical independent variable.
...
Let us assume that smoking status is coded as smoker (1) versus non-smoker (0). Because this independent variable can only take two values, we effectively have two versions of the regression equation. The first equation \@ref(eq:regdicho1) represents all smokers, so their smoking status score is 1. The smoking status of this group has a fixed contribution to the predicted average attitude, namely $b$.
...
Therefore, it is recommended to interpret only the unstandardized regression coefficient for a dichotomous independent variable. Interpret it as the difference in average scores for two groups as we have done in the preceding paragraph.

### A categorical independent variable and dummy variables {#categorical-predictor}

How about a categorical variable, that is, a variable containing three or more groups, for example, the distinction between respondents who smoke (smokers), stopped smoking (former smokers), and respondents who never smoked (non-smokers)? Can we include a categorical variable as an independent variable in a regression model? Yes, we can but we need a trick.

```{r regression-categorical, fig.cap="What are the predictive effects of smoking status?", echo = FALSE, screenshot.opts = list(delay = 5), dev="png", out.width="775px"}
...
```{block2, type='rmdquestion'}
7. In the initial state of Figure \@ref(fig:regression-categorical), can you tell whether the attitude of smokers is significantly different from the attitude of former smokers? Select different reference groups to motivate your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.7)
```
...
In this example, smoking status is measured with three categories: (1) non-smokers, (2) former smokers, and (3) smokers? We can only include such a categorical independent variable if we change it into a set of dichotomies. 
...
Let us first ceate the equation for non-smokers. To this end, we replace both $smokesnomore$ and $smoking$ by 0. As a result, both dummy variables drop from the equation [Eq. \@ref(eq:regcat1)], so the constant is the predicted atitude for non-smokers. The non-smokers are our _reference group_ because they are not represented by a dummy variable in the equation.

\begin{equation}
\small
\begin{split}
  attitude &= constant + b_1*smokesnomore + b_2*smoking \\
  attitude &= constant + b_1*0 + b_2*0 \\
  attitude &= constant
\end{split}
(\#eq:regcat1) 
\normalsize
...
For former smokers, we plug in 1 for $smokesnomore$ and 0 for $smoking$. The predicted atitude for former smokers equals the constant plus the unstandardized regression coefficient for the $smokesnomore$ dummy variable ($b_1$), see Equation \@ref(eq:regcat2). Remember that the constant represents the non-smokers (reference group), so the unstandardized regression coefficient $b_1$ for the $smokesnomore$ dummy variable shows us the difference between former smokers and non-smokers: How much more positive or more negative the average attitude towards smoking is among former smokers than among non-smokers.

\begin{equation}
\small
\begin{split}
  attitude &= constant + b_1*smokesnomore + b_2*smoking \\
  attitude &= constant + b_1*1 + b_2*0 \\
  attitude &= constant + b_1
\end{split}
(\#eq:regcat2) 
\normalsize
\end{equation}

Finally, for smokers, we plug in 0 for $smokesnomore$ and 1 for $smoking$ [\@ref(eq:regcat3)]. The predicted atitude for smokers equals the constant plus the unstandardized regression coefficient for the $smoking$ dummy variable ($b_2$). This regression coefficient, then, represents the difference in average attitude between smokers and non-smokers (reference group).

\begin{equation}
\small
\begin{split}
  attitude &= constant + b_1*smokesnomore + b_2*smoking \\
  attitude &= constant + b_1*0 + b_2*1 \\
  attitude &= constant + b_2
\end{split}
(\#eq:regcat3) 
\normalsize
\end{equation}

The interpretation of the effects (regression coefficients) for the included dummies is similar to the interpretation for a single dichotomous independent variable such as smoker versus non-smoker. It is the difference between the average score of the group coded 1 on the dummy variable and the average score of the reference group (coded 0) on the dependent variable. But now we compare to the group scoring 0 on all dummy variables that represent the categorical independent variable.
...
Finally, note that we should not include all three dummy variables in the regression model [Eq. \@ref(eq:regcat)]. We can already identify the non-smokers, because they score 0 on both the $smokesnomore$ and $smoking$ dummy variables. Adding the $neversmoked$ dummy variable to the regression model is like including the same independent variable twice. How can the estimation process decide which of the two identical independent variable is responsible for the effect? It can't decide, so the estimation process fails. If this happens, the independent variables are said to be perfectly _multicollinear_.

### Visualizing predictions in a multiple regression model

If we focus on the relation between one independent variable and the dependent variable, the predicted values can be represented by a straight line in a scatter plot. By convention, we display the independent variable on the horizontal axis and the dependent variable on the vertical axis of a plot. 

In a simple regression model, we have only one independent variable, so we can draw one regression line. In a multiple regression model, however, we have several independent variables. We can draw regression lines for each independent variable and, importantly, we can draw different regression lines for the same independent variable. Let us have a look at the regression lines that we can draw for the effect of exposure on attitude in a model that also contains smoking status and contact with smokers as independent variables (Fig. \@ref(fig:regression-predict)).

```{r regression-predict, fig.cap="How do predictions based on exposure depend on values of smoking status and smoker contact?", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
...
```{block2, type='rmdquestion'}
9. What happens in Figure \@ref(fig:regression-predict) if you change smoking status from non-smoker to smoker? Can you explain the size of changes? [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.9)
```

<A name="question8.1.10"></A>
```{block2, type='rmdquestion'}
10. Using the (red) multiple regression equation in Figure \@ref(fig:regression-predict), calculate the constant of the regression equation representing the effect of exposure on attitude for non-smokers who have five contacts with smokers. [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.10)
```

<A name="question8.1.11"></A>
```{block2, type='rmdquestion'}
11. Why does the slope of the regression line remain the same regardless of the smoking status or contact value selected? [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.11)
```

If we want to draw the regression line for the predictive effect of one independent variable in a multiple regression model---we will refer to this variable as the _predictor_---we regard the other independent variables as covariates. Let us define a _covariate_ as a variable that may predict the dependent variable but is not central to our analysis, so we mainly want to control for its effects (more on this in Chapter \@ref(mediation)). For example, if we focus on the predictive effect of exposure on attitude towards smoking, exposure is our predictor and smoking status and contact with smokers are covariates. 

```{block2, type='rmdimportant'}
A _predictor_ is the independent variable that is central to our analysis.

A _covariate_ is an independent variable that is not central to our analysis.
```
...
To draw the regression line for the effect of exposure on attitude towards smoking, we must select a value for each covariate. If we would not do so, we have more than one variable that is allowed to vary but we can only display one predictor on the horizontal axis of our plot.
...
```{r resid-normal, fig.cap="What are the residuals and how are they distributed?", echo=FALSE, screenshot.opts = list(delay = 5), dev="png", out.width="775px"}
...
```{block2, type='rmdquestion'}
12. What do the lines between dots and regression line represent in the scatter plot of Figure \@ref(fig:resid-normal)? [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.12)
```

<A name="question8.1.13"></A>
```{block2, type='rmdquestion'}
13. What is the relation between the scatter plot and the histogram? Can you point out the dot in the scatter plot that belongs to the leftmost bar in the histogram? Tip:  [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.13)
```

<A name="question8.1.14"></A>
```{block2, type='rmdquestion'}
14. Draw some new samples. Are the residuals always normally distributed? [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.14)
```
...
The regression models that we discuss assume that the association between the independent variable and dependent variables is linear. Residuals of the regression model help us to see whether this is the case.

```{r pred-linearity, fig.cap="How do residuals tell us whether the relation is linear?", echo=FALSE, screenshot.opts = list(delay = 5), dev="png", out.width="775px"}
...
```{block2, type='rmdquestion'}
15. Which dot in the plot of residuals (Figure \@ref(fig:pred-linearity) bottom) corresponds with the left-most observation (dot) in the scatter plot of attitude by exposure (Figure \@ref(fig:pred-linearity) top)? Drag your mouse around the left-most dot while pressing the left mouse button to check your choice. Repeat for more dots until you understand the relation between the two plots. [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.15)
```

<A name="question8.1.16"></A>
```{block2, type='rmdquestion'}
16. Select a U-shaped curve in Figure \@ref(fig:pred-linearity). Explain how the plot of residuals tells you that the association is not linear. Do the same for a curved association. [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.16)
```

The other two assumptions that we use tell us about problems in our model rather than problems in our statistical inferences. Our linear regression model assumes a linear effect of the independent variables on the dependent variable (_linearity_) and it assumes that we can predict the dependent variable equally well or equally badly for all levels of the dependent variable (_homoscedasticity_). 

The regression models that we estimate assume a linear model. This means that an additional unit of the independent variable always increases the predicted value by the same amount. If our regression coefficient for the effect of exposure on attitude is -0.25, an exposure score of one predicts a 0.25 more negative attitude towards smoking than zero exposure. Exposure score five predicts the same difference in comparison to score four as exposure score ten in comparison to exposure score nine, and so on. Because of the linearity assumption, we can graph a regression model as a straight line.

The relation between an independent and dependent variable, for example, exposure and attitude towards smoking, need not be linear. It can be curved or have some other fancy shape. Then, the linearity assumption is not met. A straight regression line does not nicely fit the data.

We can see this in a graph showing the (standardized) residuals (vertical axis) against the (standardized) predicted values of the dependent variable (on the horizontal axis). Note that the residuals represent prediction errors. If our regression predictions are systematically too low at some levels of the dependent variable and too high at other levels, the residuals are not nicely distributed around zero for all predicted levels of the dependent variable. This is what you see if the association is curved or U-shaped.
...
```{r pred-homoscedasticity, fig.cap="How do residuals tell us that we predict all values equally well?", echo=FALSE, screenshot.opts = list(delay = 5), dev="png", out.width="775px"}
...
<A name="question8.1.17"></A>
```{block2, type='rmdquestion'}
17. What strikes you about the residuals in Figure \@ref(fig:pred-homoscedasticity)? [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.17)
```

<A name="question8.1.18"></A>
```{block2, type='rmdquestion'}
18. What happens if you move the slider to the far left? [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.18)
```

<A name="question8.1.19"></A>
```{block2, type='rmdquestion'}
19. At which slider position are all attitude levels predicted equally well or equally badly? [<img src="icons/2answer.png" width=115px align="right">](#answer8.1.19)
```
...
* This equals the constant of a regression equation because we plug in zero (0) as the value of exposure, so the regression equation simplifies to $attitude = 1.6 + b * 0 = 1.6 + 0$.
...
* $attitude = 1.6 + (-0.6) * 10 = 1.6 + -6.0 = -4.4$ 
...
* With equations:
* Smokers average: $attitude = constant + b * status = constant + b * 1 = constant + b$
* Non-smokers average: $attitude = constant + b * status = constant + b * 0 = constant$
* Smokers - Non-smokers: $(constant + b) - constant = b$ [<img src="icons/2question.png" width=161px align="right">](#question8.1.5)
```
...
group is in comparison to the reference group. You may check that *b* is equal
to the average of the non-reference group, e.g., former smokers, minus the
average of the reference group, e.g., non-smokers.
* The associated p value tells us how uncertain we are that there truly is a
mean difference in the population. It tests the null hypothesis that the two
groups have the same mean score on the dependent variable in the population.
...
* In the initial state of this figure, non-smokers are the
reference group. The p values, then, are associated with differences between on
the one hand non-smokers and on the other hand people who stopped
smoking or are still smoking. The comparison between the latter two is not included.
...
(difference between group means) and on the standard error. The latter depends, among other things, on the sample size per group. All of these aspects may vary across groups, so we
cannot guess the p value of the mean difference between two groups.
...
selecting a different group in the drop-down list. [<img src="icons/2question.png" width=161px align="right">](#question8.1.7)
```
...
* The multiple regression line moves up.
* The change from non-smoker to smoker adds 0.5 to the predicted values. The line moves up 0.5 points on the attitude scale. [<img src="icons/2question.png" width=161px align="right">](#question8.1.9)
```
...
* Copy the red equation to your scrap paper and replace $Smoker$ by 0 (for non-smokers) and replace $contact$ by 2 (for people who have two contacts with smokers).
* $attitude=0.65−0.6∗exposure+0.5∗0+0.15∗2$
* $attitude=0.65−0.6∗exposure+0+0.30$
* $attitude=0.95−0.6∗exposure$
* The constant is 0.95. [<img src="icons/2question.png" width=161px align="right">](#question8.1.10)
```
...
* The slope of the regression line remains the same because plugging in a value for smoking status or contact only changes the constant, not the regression coefficient of exposure. 
* The constant of the simplified regression line is the predicted attitude if exposure is zero. A higher constant moves the regression line up, a lower constant moves it down. [<img src="icons/2question.png" width=161px align="right">](#question8.1.11)
```
...
independent variable score (exposure) of the respondent. [<img src="icons/2question.png" width=161px align="right">](#question8.1.12)
```
...
histogram. [<img src="icons/2question.png" width=161px align="right">](#question8.1.13)
```
...
* No, sometimes the distribution is clearly skewed (asymmetrical). [<img src="icons/2question.png" width=161px align="right">](#question8.1.14)
```
  
<A name="answer8.1.15"></A>
```{block2, type='rmdanswer'}
Answer to Question15. 
...
left-most observation in the residuals plot. [<img src="icons/2question.png" width=161px align="right">](#question8.1.15)
```
...
may underestimate as well as overestimate the dependent variable everywhere. [<img src="icons/2question.png" width=161px align="right">](#question8.1.16)
```
  
<A name="answer8.1.17"></A>
```{block2, type='rmdanswer'}
...
exposure scores than for participants with high exposure scores. [<img src="icons/2question.png" width=161px align="right">](#question8.1.17)
```
  
<A name="answer8.1.18"></A>
```{block2, type='rmdanswer'}
...
well. [<img src="icons/2question.png" width=161px align="right">](#question8.1.18)
```
  
<A name="answer8.1.19"></A>
```{block2, type='rmdanswer'}
...
* This is how we like the plot to look. [<img src="icons/2question.png" width=161px align="right">](#question8.1.19)
```
...
knitr::include_url("https://www.youtube.com/embed/euVRsi-NpKk", height = "360px")
...
knitr::include_url("https://www.youtube.com/embed/hiYbfhkKODE", height = "360px")
...
knitr::include_url("https://www.youtube.com/embed/QB4-LJVJPu4", height = "360px")
...
knitr::include_url("https://www.youtube.com/embed/QSipdx57t3U", height = "360px")
...
knitr::include_url("https://www.youtube.com/embed/_9u-VVSh7iU", height = "360px")
...
```{block2, type='rmdquestion'}
1. Use the data in [smokers.sav](http://82.196.4.233:3838/data/smokers.sav) to predict the attitude towards smoking from exposure to an anti-smoking campaign. Check the assumptions and interpret the results. [<img src="icons/2answer.png" width=115px align="right">](#answer8.2.1)
```

<A name="question8.2.2"></A>
```{block2, type='rmdquestion'}
2. Add smoking status (variable _status3_), and contact with smokers as predictors to the regression model of Exercise 1. Compare the effect of exposure between the two regression models. What is the difference and why is there a difference? [<img src="icons/2answer.png" width=115px align="right">](#answer8.2.2)
```

<A name="question8.2.3"></A>
```{block2, type='rmdquestion'}
3. The data set [children.sav](http://82.196.4.233:3838/data/children.sav) contains information about media literacy of children and parental supervision of their media use. Are the two related? Check the assumptions and interpret the results. [<img src="icons/2answer.png" width=115px align="right">](#answer8.2.3)
```

<A name="question8.2.4"></A>
```{block2, type='rmdquestion'}
4. How well can we predict brand awareness with ad exposure? Use [consumers.sav](http://82.196.4.233:3838/data/consumers.sav) to answer this question. [<img src="icons/2answer.png" width=115px align="right">](#answer8.2.4)
```
...
dependent variable, so the association seems to be linear.
...
well, *R*^2^ = .17, *F* (1, 83) = 16.73, *p* < .001.
...
strong effect (*b\** = -.41). [<img src="icons/2question.png" width=161px align="right">](#question8.2.1)
```
...
attitude towards smoking, which is very much for a social scientific model, *R*^2^
...
positive attitude. Of the two, exposure (_b\*_ = -0.44) is a better predictor
than contact with smokers (_b\*_ = 0.20).
* Former smokers are on average much (2.85 points) more negative about smoking
than non-smokers; smokers are on average only 0.20 points below the
...
the model should not be linear. [<img src="icons/2question.png" width=161px align="right">](#question8.2.2)
```
...
the predicted scores but there are perhaps too few observations for low and
...
regression model if you use supervision or media literacy as dependent variable. [<img src="icons/2question.png" width=161px align="right">](#question8.2.3)
```
...
predicted values but the number of observations is quite small, so this is
difficult to check.
  
Interpret the results:  
  
Ad exposure predicts about one fifth of the variation in brand awareness
scores, *R*^2^ = .21, *F* (1, 60) = 15.87, *p* < .001.
The predictive effect of exposure to brand advertisements is moderately strong
(*b\** = 0.46). An additional unit of exposure increases the predicted brand
awareness with 0.4 points, *t* = 3.98, *p* < .001, 95%[0.22; 0.67]. [<img src="icons/2question.png" width=161px align="right">](#question8.2.4)
```

## Different Lines for Different Groups {#categoricalmoderator}

If we have a categorical independent variable, for instance, people who are exposed to a campaign versus people who are not exposed to this campaign, and we want to determine its effect on a numerical variable, for example, attitude towards smoking, we compare group means. The difference between group means is the main effect of the categorical variable. For example, the average attitude towards smoking is 0.5 points less positive for people who are exposed to the campaign than for people who are not exposed to it. 

In analysis of variance (Chapter \@ref(anova)), the main effect of campaign exposure is the average effect for all people regardless of their other characteristics or the contexts that they are in. In other words, a main effect is the overall average difference in attitude between people who are exposed to the campaign and those who are not.

What if the effect of campaign exposure on attitude may be different in different contexts, e.g., for people who smoke themselves, used to smoke, or never smoked (*smoking status*)? To model this, we add an interaction effect to the main effects (compare Chapter \@ref(anova)).

The interaction effect tells us whether the attitude difference between, on the one hand, people who are exposed to the campaign and, on the other hand, people who are not exposed to the campaign is different for smokers, former smokers, and non-smokers. In a conceptual diagram, the interaction effect is represented by an arrow pointing at another arrow. The moderator (smoking status) changes the relation between the predictor (campaign exposure) and the dependent variable (attitude towards smoking).
...
ggplot(variables, aes(x, y)) + 
  geom_segment(aes(x = variables$x[1], y = variables$y[1], xend = variables$x[3], yend = variables$y[1]), arrow = arrow(length = unit(0.04, "npc"), type = "closed")) + 
...
What if campaign exposure is a numerical variable, for example, counting the number of times the respondent has been exposed to the campaign? How can we investigate whether the effect of this numerical predictor on attitude is moderated by the respondent's smoking status?

Analysis of variance (ANOVA), as discussed in Chapter \@ref(anova), investigates the effects of categorical variables on a numerical dependent variable. It cannot handle numerical predictors or numerical covariates. Although there are ways to include numerical covariates in analysis of variance, for instance, in the analysis of covariance (ANCOVA), we use regression analysis if we have at least one numerical predictor or covariate and a numerical dependent variable. 

In the current section, we discuss regression models with a numerical predictor and a categorical moderator. A later section (Section \@ref(cont-moderator-regression)) presents regression models in which both the predictor and moderator are numerical.
...
#Cleanup (partial).
rm(smokers, model_1, ci, results_2, model_2, z_attitude, z_status2, z_expostatus2, z_exposure, z_contact)
```
...
```{r dich-moderator-graph, echo=FALSE, message=FALSE, warning=FALSE, fig.asp=0.5, fig.cap="The effects of exposure to the anti-smoking campaign on attitude towards smoking among smokers and non-smokers."}
# Graph of regression lines calculated in chunk dich-moderator-output.
# Plot.
ggplot() +
  geom_segment(aes(x = 0, xend = 10, 
                   y = results[1,1] + results[5, 1] * contact_avg, 
                   yend = results[1,1] + 10*results[2,1] + results[5, 1] * contact_avg,
                   colour = "Non-smokers"),
               size = 1
               ) +
  geom_segment(aes(x = 0, xend = 10, 
                   y = results[1,1] + results[3,1] + results[5, 1] * contact_avg, 
                   yend = results[1,1] + results[3,1] + 10*(results[2,1] + results[4,1]) + + results[5, 1] * contact_avg,
               colour = "Smokers"),
               size = 1
               ) +
...
Remember that the regression coefficient for the interaction variable expresses the difference between the slope for the indicated group, e.g., smokers, and the slope for the reference group, e.g., non-smokers. If this difference is zero, as stated by the null hypothesis, the two groups have the same slope, so the effect is not moderated by the group variable. The regression lines for the two groups are (more or less) parallel.
...
The effect of contact with smokers, however, is not included in an interaction in the model summarized by Table \@ref(tab:dich-moderator-output). For this predictor we can interpret the standardized effect _b_* = `r  report_n(results[5, 3], digits = 2)`, which SPSS reports as _Beta_.
...
What if we have three or more groups on our moderator? For example, smoking status measured with three categories: (1) never smoked, (2) formerly smoked, (3) currently smoking? Does the effect of exposure on attitude vary between non-smokers, stopped smoking, and are still smoking?
...
```{r common-support, fig.cap="How well do the observations cover the predictor within each category of smoking status?", echo=FALSE, screenshot.opts = list(delay = 5), dev="png", out.width="775px"}
...
```{block2, type='rmdquestion'}
3. What does the histogram represent in Figure \@ref(fig:common-support)? [<img src="icons/2answer.png" width=115px align="right">](#answer8.3.3)
```

<A name="question8.3.4"></A>
```{block2, type='rmdquestion'}
4. Are the exposure values nicely spread for each smoke status group? Inspect each smoke status group separately with the "Show groups" option. [<img src="icons/2answer.png" width=115px align="right">](#answer8.3.4)
```

<A name="question8.3.5"></A>
```{block2, type='rmdquestion'}
5. What is the problem if exposure scores are not nicely spread over the same range for all smoke status groups? [<img src="icons/2answer.png" width=115px align="right">](#answer8.3.5)
```
...
It is recommended to check the number of observations per value of the moderator. For a categorical moderator, such as smoking status, a scatter plot of the dependent variable (vertical axis) by predictor (horizontal axis) with dots coloured according to the moderator category may do the job. Check that there are observations for more or less all values of the predictor. In Figure \@ref(fig:scatter-moderated), observations in each moderator category (dot colour) range from low to high predictor values.
...
* In all other situations, there is moderation. [<img src="icons/2question.png" width=161px align="right">](#question8.3.1)
```
...
parallel to the other two lines. [<img src="icons/2question.png" width=161px align="right">](#question8.3.2)
```
...
(smoking status). [<img src="icons/2question.png" width=161px align="right">](#question8.3.3)
```
...
* For one moderator (smoking status) group, the predictor (exposure) may have scores for the entire range, that is, from 0 to 10. But scores may also be available only for part of this range, for example, from 1 to 6 or from 4 to 8. [<img src="icons/2question.png" width=161px align="right">](#question8.3.4)
```
...
regression effect for this group is not to be trusted. [<img src="icons/2question.png" width=161px align="right">](#question8.3.5)
```
...
knitr::include_url("https://www.youtube.com/embed/8iB4LiTWSVY", height = "360px")
...
knitr::include_url("https://www.youtube.com/embed/ZBIm1rrdXZE", height = "360px")
...
knitr::include_url("https://www.youtube.com/embed/JyiFKgaF8-M", height = "360px")
...
knitr::include_url("https://www.youtube.com/embed/JznCrf0lCIU", height = "360px")
...
model, *R*^2^ = .26, *F* (4, 80) = 7.10, *p* < .001.
...
exposed to the campaign, on average a circa 2 (0.5 to 3.4) points more
positive the attitude, and this difference is statistically significant, 
*b* = 1.98, *t* = 2.72, *p* = .008, 95%CI[0.53, 3.43].
...
or average effect as in analysis of variance.
...
can interpret the standardized regression coefficient for this effect. [<img src="icons/2question.png" width=161px align="right">](#question8.4.1)
```
...
* Plug in the average value of the covariate: 
...
* For smokers, the predictive effect of campaign exposure on smoking attitude
is more strongly negative (*b* = -0.45) than for non-smokers (*b* = -0.12). [<img src="icons/2question.png" width=161px align="right">](#question8.4.2)
```
...
much better (*R*^2^ = 0.70) than with the two groups in Exercise 1 (*R*^2^ = .26).
...
```{block2, type='rmdquestion'}
1. The regression line depicted in Figure \@ref(fig:continuous-moderator) represents the conditional effect of exposure on attitude for the value of contact with smokers selected with the slider. How many different conditional effects are there? [<img src="icons/2answer.png" width=115px align="right">](#answer8.5.1)
```

<A name="question8.5.2"></A>
```{block2, type='rmdquestion'}
2. Is the effect of campaign exposure on attitude towards smoking always negative? Or does more exposure lead to a more positive attitude (higher score) in some cases? If so, in which cases? [<img src="icons/2answer.png" width=115px align="right">](#answer8.5.2)
```

<A name="question8.5.3"></A>
```{block2, type='rmdquestion'}
3. How much does the slope increase if the moderator value is changed from 0 to 1? And how much if it changes from 6 to 7? [<img src="icons/2answer.png" width=115px align="right">](#answer8.5.3)
```

People hanging around a lot with smokers are likely to have a more positive attitude towards smoking than people who have little contact with smokers. After all, people who really hate smoking will avoid meeting smokers. This is a main effect of contact with smokers on attitude towards smoking.
...
```{block2, type='rmdquestion'}
4. What is the correct interpretation of the estimated regression coefficient of exposure in Figure \@ref(fig:mean-centering-moderator)? [<img src="icons/2answer.png" width=115px align="right">](#answer8.5.4)
```

<A name="question8.5.5"></A>
```{block2, type='rmdquestion'}
5. What happens to the red regression line and the regression equation if you subtract the mean (*M*) from the respondents' contact with smokers scores? Thus, you mean-center the moderator Contact. Use the slider **Center Contact. Subtract from Contact:** in Figure \@ref(fig:mean-centering-moderator) to check your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer8.5.5)
```

<A name="question8.5.6"></A>
```{block2, type='rmdquestion'}
6. If Contact is mean-centered, the regression coefficient of exposure represents the effect of exposure on attitude for respondents with a particular score on the original Contact variable. What is this original Contact score? Use the slider **Adjust the value of Contact (Moderator):** to check your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer8.5.6)
```
...
```{block2, type='rmdquestion'}
7. Select one or more options in Figure \@ref(fig:continuous-interaction-visualization) to represent regression lines predicting attitude from exposure at different values of the moderator (contact with smokers). Respondents with moderator values close to the selected value are coloured. Which moderator values would you pick to communicate the results of moderation? Motivate your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer8.5.7)
```
...
source("report_n.R")
# Partial cleanup.
rm(model_1, ci, results_2, model_2, z_attitude, z_contact, z_status2, z_expocontact, z_exposure)
```
...
can select, so there are 21 different regression lines that can be depicted. [<img src="icons/2question.png" width=161px align="right">](#question8.5.1)
```
...
> contact = 0.26 / 0.04 = 6.5 [<img src="icons/2question.png" width=161px align="right">](#question8.5.2)
```
...
contact levels six and seven. [<img src="icons/2question.png" width=161px align="right">](#question8.5.3)
```
...
respondents who have no (zero) contacts with smokers. [<img src="icons/2question.png" width=161px align="right">](#question8.5.4)
```
...
almost horizontal. [<img src="icons/2question.png" width=161px align="right">](#question8.5.5)
```
...
on the new mean-centered variable, they have become the reference group. [<img src="icons/2question.png" width=161px align="right">](#question8.5.6)
```
...
itself. [<img src="icons/2question.png" width=161px align="right">](#question8.5.7)
```
...
Exposure, in our example, has a negative predictive effect on attitude towards smoking (_b_ = `r report_n(results[2,1])`, `r ifelse(results[2,4] < .0005, "p < .001", paste0("p = ", report_n(results[2,4], digits=3)))`) for non-smokers with average contacts with smokers, t = `r report_n(results[2,3])`, `r ifelse(results[2,4] < .0005, "p < .001", paste0("p = ", report_n(results[2,4], digits=3)))`, 95%CI[`r report_n(ci[2,1])`, `r report_n(ci[2,2])`]. Note that SPSS does not report the degrees of freedom for the t test on  regression coefficient, so we cannot report them.

```{r report-moderation-table, echo=FALSE}
# Table.
options(knitr.kable.NA = '')
knitr::kable(table, booktabs = TRUE, align = c("l", "c"),
             caption = "Predicting attitude towards smoking. Results in APA6 style. Exposure and contact are mean-centered.") %>%
...
ggplot(df, aes(x = exposure_mc, y = attitude)) +
  ...
  xlab("Exposure (mean-centered)") +
  ylab("Attitude towards smoking") +
  scale_color_manual(values = c(" M-SD" = cl[5], "M" = cl[4], "M+SD" = cl[3])) +
  theme(legend.position = "bottom")
...
Do not report that common support is good. If it is bad, warn the reader that we cannot fully trust the estimated moderation because we do not have a nice range of predictor values within each level of the moderator. If the predictor is support only within a restricted range, you may report this range.
...
<A name="question8.7.1"></A>
```{block2, type='rmdquestion'}
1. With the data in [smokers.sav](http://82.196.4.233:3838/data/smokers.sav), check if the effect of campaign exposure on attitude towards smoking depends on the contacts that people have with smokers. __For now, do not mean-center the variables.__ Control for the respondent's smoking status (_status2_). Interpret the regression coefficients and check the assumptions of the regression model. [<img src="icons/2answer.png" width=115px align="right">](#answer8.7.1)
```

<A name="question8.7.2"></A>
```{block2, type='rmdquestion'}
2. Visualize the moderating effect of contact with smokers on the exposure effect (Exercise 1) in a scatter plot with three regression lines. Explain the information conveyed by the plot to your reader. [<img src="icons/2answer.png" width=115px align="right">](#answer8.7.2)
```

<A name="question8.7.3"></A>
```{block2, type='rmdquestion'}
3. Mean-center the predictor and moderator and repeat the regression analysis of Exercise 1. Explain the differences in the results. [<img src="icons/2answer.png" width=115px align="right">](#answer8.7.3)
```

<A name="question8.7.4"></A>
```{block2, type='rmdquestion'}
4. Check common support of the predictor for the moderator. Divide the moderator into three groups, each containing about a third of all observations. [<img src="icons/2answer.png" width=115px align="right">](#answer8.7.4)
```

<A name="question8.7.5"></A>
```{block2, type='rmdquestion'}
5. Let us hypothesize that children's media literacy depends on sex, age, and parental supervision. Is the effect of parental supervision moderated by the child's age?
    ...
    Report the results as required in this course (APA6), include a moderation plot, and discuss common support. [<img src="icons/2answer.png" width=115px align="right">](#answer8.7.5)
```

<A name="question8.7.6"></A>
```{block2, type='rmdquestion'}
6. Is the effect of parental supervision moderated by sex? Use the data of Exercise 5 to answer this question. You may omit the age predictor from the model. Again, illustrate your answer with a moderation plot. [<img src="icons/2answer.png" width=115px align="right">](#answer8.7.6)
```

### Answers {-}

<A name="answer8.7.1"></A>
```{block2, type='rmdanswer'}
Answer to Exercise 1. 
...
negative for adults who have no exposure to the anti-smoking campaign, *b* =
...
results states that the effect may go either way: positive or negative. [<img src="icons/2question.png" width=161px align="right">](#question8.7.1)
```

<A name="answer8.7.2"></A>
```{block2, type='rmdanswer'}
Answer to Exercise 2. 
...
slightly stronger (more negative) for adults with fewer contacts with smokers. [<img src="icons/2question.png" width=161px align="right">](#question8.7.2)
```

<A name="answer8.7.3"></A>
```{block2, type='rmdanswer'}
Answer to Exercise 3. 
...
zero in the population. [<img src="icons/2question.png" width=161px align="right">](#question8.7.3)
```

<A name="answer8.7.4"></A>
```{block2, type='rmdanswer'}
Answer to Exercise 4. 
...
* Common support of the exposure predictor is poor at low contact levels. Especially low exposure hardly occurs at low contact level. This explains the high standard error for the exposure effect in Exercise 1 as explained in Exercise 3. [<img src="icons/2question.png" width=161px align="right">](#question8.7.4)
```

<A name="answer8.7.5"></A>
```{block2, type='rmdanswer'}
...
illustrates the absence of a substantial interaction effect. [<img src="icons/2question.png" width=161px align="right">](#question8.7.5)
```

<A name="answer8.7.6"></A>
```{block2, type='rmdanswer'}
...
media literacy for boys, *b* = 0.39, *t* = 2.99, *p* = .004, 95%CI[0.13; 0.65]. This
effect is weaker for girls, *b* = 0.26, but the difference between boys and girls
...
medliter = 4.499 + 0.255\*supervision_c [<img src="icons/2question.png" width=161px align="right">](#question8.7.6)
```
...
```{r regression-predict2, fig.cap="How do predictions based on exposure depend on values of smoking status and smoker contact?", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
# Goal: sensitize student to the notion that prediction in a multiple regression
# model requires selecting one independent variable and fixed values for the other
# independent variables. Without moderation, the fixed values only change the vertical
# position of the line (that is, the constant) but not the slope.
...
<A name="question8.8.1"></A>
```{block2, type='rmdquestion'}
1. Do smokers or non-smokers have a more positive attitude towards smoking according to Figure \@ref(fig:regression-predict)? [<img src="icons/2answer.png" width=115px align="right">](#answer8.8.1)
```

<A name="answer8.8.1"></A>
...
* If you select the right values in the figure, the (blue) simple regression equation will read $attitude=1.75-0.6*exposure$. If exposure is zero, the predicted attitude equals the constant, which is 1.75.
  [<img src="icons/2question.png" width=161px align="right">](#question8.8.2)
```

Figure \@ref(fig:moderator-overview) again shows the relation between exposure to an anti-smoking campaign and attitude towards smoking. This time, however, the effect of exposure on attitude is moderated by daily contacts with people who smoke.
...
<A name="question8.8.4"></A>
```{block2, type='rmdquestion'}
4. What does the red line in Figure \@ref(fig:moderator-overview) mean? [<img src="icons/2answer.png" width=115px align="right">](#answer8.8.4)
```

<A name="question8.8.5"></A>
```{block2, type='rmdquestion'}
5. What happens if you change the position on the slider? Explain your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer8.8.5)
```

<A name="question8.8.6"></A>
```{block2, type='rmdquestion'}
6. Why does _contact_ (with smokers) appear in between brackets together with the regression coefficient for exposure in the regression equation? [<img src="icons/2answer.png" width=115px align="right">](#answer8.8.6)
```

<A name="question8.8.7"></A>
```{block2, type='rmdquestion'}
7. Which of the regression coefficients represent(s) a conditional effect? Explain your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer8.8.7)
```

<A name="question8.8.8"></A>
```{block2, type='rmdquestion'}
8. What is the null hypothesis of a significance test on the interaction effect ($b_3$)? [<img src="icons/2answer.png" width=115px align="right">](#answer8.8.8)
```
...
* In this model, contact with smokers has a positive effect regardless of a person's exposure and smoking status because the effect of contact is not moderated by exposure or smoking status. There is no interaction variable in the equation. [<img src="icons/2question.png" width=161px align="right">](#question8.8.3)
```

<A name="question8.8.2"></A>
```{block2, type='rmdquestion'}
2. What is the predicted attitude for smokers who are not exposed to the campaign and who have 4 contacts with smokers? Use the equation in Figure \@ref(fig:regression-predict) and see if you can easily check your answer in this figure. [<img src="icons/2answer.png" width=115px align="right">](#answer8.8.2)
```

<A name="question8.8.3"></A>
```{block2, type='rmdquestion'}
3. Do people with more contacts with smokers have a more positive or a more negative attitude towards smoking? Does this apply to all people or only to people with a particular smoking status or a particular level of exposure? [<img src="icons/2answer.png" width=115px align="right">](#answer8.8.3)
```

<A name="answer8.8.4"></A>
```{block2, type='rmdanswer', echo=ch8}
Answer to Question 4. 
...
who have no contact with smokers. [<img src="icons/2question.png" width=161px align="right">](#question8.8.4)
```

<A name="answer8.8.5"></A>
```{block2, type='rmdanswer', echo=ch8}
Answer to Question 5. 
...
steep and at some point changes into an increase from left to right. [<img src="icons/2question.png" width=161px align="right">](#question8.8.5)
```

<A name="answer8.8.6"></A>
```{block2, type='rmdanswer', echo=ch8}
Answer to Question 6. 
...
slope of the conditional effect of exposure. [<img src="icons/2question.png" width=161px align="right">](#question8.8.6)
```

<A name="answer8.8.7"></A>
```{block2, type='rmdanswer', echo=ch8}
Answer to Question 7. 
...
predictor. [<img src="icons/2question.png" width=161px align="right">](#question8.8.7)
```

<A name="answer8.8.8"></A>
```{block2, type='rmdanswer', echo=ch8}
Answer to Question 8. 
...
the same at all levels of the moderator in the population. [<img src="icons/2question.png" width=161px align="right">](#question8.8.8)
```
...
* To interpret moderation, describe the effects (slopes, unstandardized regression coefficients) and visualize the regression lines for different groups. For a numerical variable, select some interesting levels of the moderator, such as the mean and one standard deviation below or above the mean.

* Warn the reader if the predictor scores are not nicely distributed for all groups or levels (no common support). 

* Don't use the standardized regression coefficients (Beta) for interaction variables or for dummy variables in SPSS.