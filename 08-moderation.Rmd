...
model contains only one independent variable (simple regression model).
```
...
independent variable score (exposure) of the respondent.
```
...
```{r eval=FALSE}
* No, sometimes the distribution is clearly skewed (asymmetrical).
```
...
may underestimate as well as overestimate the dependent variable everywhere.
```
...
2. The line for a simple regression of attitude on exposure (with just one independent variable) is displayed in blue in Figure \@ref(fig:regression-predict). Can you make the multiple regression line equal to the simple regression line? If so, at what covariate values? If not, why not?
...
regression model if you use supervision or media literacy as dependent variable.
```

4. How well can we predict brand awareness with ad exposure? Use [consumers.sav](http://82.196.4.233:3838/data/consumers.sav) to answer this question.
...
scores, R2 = .21, F (1, 60) = 15.87, p < .001.
...
awareness with 0.4 points, t = 3.98, p < .001, 95%[0.22; 0.67].
```
...
results states that the effect may go either way: positive or negative.
```

2. Visualize the moderating effect of contact with smokers on the exposure effect (Exercise 1) in a scatter plot with three regression lines. Explain the information conveyed by the plot to your reader
...
4. Check common support of the predictor for the moderator. Divide the moderator into three groups, each containing about a third of all observations.
...
medliter = 4.499 + 0.255*supervision_c  
```
...
slope of the conditional effect of exposure.
```

4. Which of the regression coefficients represent(s) a conditional effect? Explain your answer.
```{r eval=FALSE, echo=ch8}
...
the same at all levels of the moderator in the population.
```
