...
sample statistic, for example, the sample mean, transformed into a t test
statistic.
```
...
Conditions often include sample size (Table \@ref(tab:thumbsize) reproduces the sie requirements from Table \@ref(tab:thumb)). If you plan to do a t-test, either on its own or in post-hoc tests after analysis of variance, each group should contain more than thirty cases. So if you plan on doing t-tests, recruit more than thirty subjects for each experimental group or more than thirty respondents for each group in your survey and you are fine. Well, if you have to reckon with non-response, that is, sampled subjects or respondents unwilling to participate in your research, you should recruit more subjects or respondents to have more than thirty observations in the end.  
...
# Divide the first by the second, for example, 1.880 / 1.033 = 1.82. This is a
# strong effect.
# 
...
1. Open data set <a href="http://82.196.4.233:3838/data/voters.sav" target="_blank">"voters.sav"</a> that contains information about the age and attitude towards immigration among a random sample of voters. What are the unstandardized and standardized effect sizes if the hypothesized average attitude towards immigrants in the population is 6.0?
...
3. What are the unstandardized and standardized effect sizes of a test in which we compare the attitude towards immigrants of young voters to the attitude of old voters? Again, use data set <a href="http://82.196.4.233:3838/data/voters.sav" target="_blank">"voters.sav"</a>.
...
significant (p = .029), so we do not assume equa variances and we use the
...
however, is not consequential to Cohen's d because sample size is relevant for
p values but not for the calculation of t values.
```
...
Effect size is also zero according to the stadard null hypotheses used for tests on the regression coefficient (_b_), _R_^2^ for the regression model, and eta^2^ for analysis of variance. As a result, we can use the standardized regression coefficient (Beta in SPSS and _b_* according to APA6), _R_^2^, and eta^2^ as standardized effect sizes.
...
1. Use the slider in Figure \@ref(fig:sample-size) to find the minimum sample size that we need for a statistically significant test result.
...
```{r eval=FALSE}
* The p values give the (two-sided) significance of a smaple with average candy
...
* The larger the sample, the samller the standard error, the narrower the
sampling distribution, the larger the probability of drawing a sample with a
particular minimum or maximum average candy weight score.
```
...
double-sided arc in the bottom graph.
```
...
Table \@ref(tab:errortable) summarizes the four possible situations that may arise if we test a null hypothesis. The null hyothesis may be true or false and we may or may not reject the null hypothesis.
...
It separates the (21%) samples with t values larger than the critical
...
that the critical z value for a one-dosed test is 1.64). This is in line with
...
Significant results means rejecting the null hypothesis, which contributes to
...
```{r sample-size-power}
#add Shiny app to determine sample size for a specified (standardized) effect size, significance level, and test power for a (simple) one-sample t test, using the pwr:: package.
...
Fortunately, we know the probability of making this error because it is the significance level that we have chosen, five percent usually. We can live with this probability of making an error if we reject the null hypothesis. So we are doubly glad: We found support for our research hypothesis _and_ we know how confident we are about this support.  
