...
1. If we draw a sample of twenty candies, which average candy weight in our sample rejects the null hypothesis according to Figure \@ref(fig:sample-size-unstand): 2.40, 2.70, or 3.05 gram?
...
1. Use the slider in Figure \@ref(fig:sample-size) to find the minimum sample size that we need for a statistically significant test result for each of the three effect sizes represented by red lines.
...
* The blue area is the probability that we draw a sample from a population
with true mean H1 that rejects the null hypothesis, which is false so we do
not make a Type II error. It is the probability that we reject a false null
hypothesis, which is the power of the test.
```
...
null hypothesis, so we can do with a smaller sample.
```
...
5. Do we need a smaller or a larger sample to achieve the specified test power for a higher significance level, for example, $\alpha = 0.10$? Move the significance level slider to check your answer.
...
upper graph are also used in the bottom graph.
```
...
8. What happens to the probability of a Type II error (the blue section in the bottom graph) if you change the significance level with the slider? 
```{r eval=FALSE, echo=TRUE}
* With a higher significance level (alpha goes up), we take a larger risk to
