...
# Interactive table with sample size as first column (initially one value: 5), effect size Cohen's d as second column (all cells are initially 'small (0.2)'), t value as calculated third column (square root of sample size times 0.2, 0.5, or 0.8), and p value (calculated, p values below .05 are printed in red) as fifth column ; after entering a new sample size (data input for integer numbers; DT:: package does not allow changing cell values?)) or a different effect size (selection list: 'small (0.2)', 'moderate (0.5)', or 'strong (0.8)'), the table is updated and sorted on sample size (can this be done with the DT:: package in Shiny?), so the resulting table shows the progression t larger samples and smaller p values
...
![R.A. Fisher](figures/epearson.jpg) Egon Pearson</td></tr></table>
...
```{r TypeI-II-errors, fig.cap="Simulation of Type I and Type II error."}
...
knitr::include_app("http://82.196.4.233:3838/type1vs2/", height="880px")
```
...
knitr::include_app("http://82.196.4.233:3838/ttest_simulation/", height=680)
```
...
```{r sample-size-power}
#add Shiny app to determine sample size for a specified (standardized) effect size, significance level, and test power fir some (simple) tests?
