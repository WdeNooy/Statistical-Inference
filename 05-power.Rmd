...
> Key concepts: minimum sample size, practically significance, unstandardized effect size, standardized effect size, Cohen’s d for means, Type I error, Type II error, test power.
...
* All sample sizes from 18 up to and including 19 serve our purpose, namely
...
```{block2, type='rmdquestion'}
2. What happens to the red, yellow, and blue surfaces if you set power to 50%? And what happens to the minimum sample size? Adjust the power slider to check your answers. [<img src="icons/2answer.png" width=115px align="right">](#answer5.4.2)
```

<A name="question5.4.3"></A>
```{block2, type='rmdquestion'}
3. Do we need a smaller or a larger sample to achieve the specified test power for a larger effect size? Move the effect size slider to check your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer5.4.3)
```

<A name="question5.4.4"></A>
```{block2, type='rmdquestion'}
4. Do we need a smaller or a larger sample to achieve the specified test power for a one-sided test instead of a two-sided test? Change the test type to check your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer5.4.4)
```

<A name="question5.4.5"></A>
```{block2, type='rmdquestion'}
5. Do we need a smaller or a larger sample to achieve the specified test power for a higher significance level, for example, $\alpha = 0.10$? Move the significance level slider to check your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer5.4.5)
```

<A name="question5.4.6"></A>
```{block2, type='rmdquestion'}
6. Why do the sampling distributions become wider if we increase effect size or significance level, or if we decrease test power? [<img src="icons/2answer.png" width=115px align="right">](#answer5.4.6)
```
...
```{r sample-size-power2, fig.cap="How does sample size affect test power?", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
# Shiny app to determine power for a specified (standardized) effect size,
# significance level, and sample size for a (simple) one-sample t test, using the
# pwr:: package.
...
```{block2, type='rmdquestion'}
7. Which minimum sample size do we need to achieve 95% test power in the situation depicted in Figure \@ref(fig:sample-size-power2)? [<img src="icons/2answer.png" width=115px align="right">](#answer5.4.7)
```

<A name="question5.4.8"></A>
```{block2, type='rmdquestion'}
8. Effects tend to be small (0.2) rather than moderate (0.5) in communication science. What is the test power for a small effect with the sample size that you obtained in Question 7 and how large must the sample be if we want 80% test power for a small effect? [<img src="icons/2answer.png" width=115px align="right">](#answer5.4.8)
```
...
* If we set power to 50%, the yellow and blue areas (including the red tail on
the right) each should occupy half of the sampling distribution that is
centred at H1.
* A smaller power implies that we run a larger risk of not rejecting a false
null hypothesis, so we can do with a smaller sample. [<img src="icons/2question.png" width=161px align="right">](#question5.4.2)
```
...
(H1) and the population mean according to the null hypothesis (H0) is
larger, and it will be more easily spotted in a smaller sample. In other words,
we have higher power with a larger effect size. We can do with a smaller sample to
achieve the original test power. [<img src="icons/2question.png" width=161px align="right">](#question5.4.3)
```
...
increases if we keep the same sample size. To maintain the original test power,
we may draw a smaller sample. [<img src="icons/2question.png" width=161px align="right">](#question5.4.4)
```
...
as a change from a two-sided test to a one-sided test. We are more likely to
reject the null hypothesis, so we are more likely to reject a false null
hypothesis, so we have higher test power. To maintain the original test power,
we may draw a smaller sample. [<img src="icons/2question.png" width=161px align="right">](#question5.4.5)
```
...
on average further away from the mean of the distribution. [<img src="icons/2question.png" width=161px align="right">](#question5.4.6)
```
...
* Increase sample size by dragging the sample size slider to the right. At sample size 53, test power is 95%. Minimum sample size for 95% test power, then, is 53. [<img src="icons/2question.png" width=161px align="right">](#question5.4.7)
```
  
<A name="answer5.4.8"></A>
```{block2, type='rmdanswer'}
Answer to Question 8. 

* Leave sample size at 53, and change the standardized effect size to 0.2 (small effect). The figure reports a test power of only 30%.
* At sample size 196, the power of a test with a small effect becomes 80%. Note that we needed a sample of just 33 observations to obtain 80% test power in case of a moderate effect. Small effects quickly require large samples! [<img src="icons/2question.png" width=161px align="right">](#question5.4.8)
```
