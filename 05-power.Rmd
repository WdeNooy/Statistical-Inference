...
```{r sample-size-unstand, fig.cap="Sampling distribution of average candy weight under the null hypothesis that average candy weight is 2.8 gram in the population.", echo=FALSE, out.width="420px", screenshot.opts = list(delay = 5), dev="png"}
...
1. If we draw a sample of twenty candies, which average candy weight in our sample rejects the null hypothesis according to Figure \@ref(fig:sample-size-unstand): 2.40, 2.70, or 3.05 gram? [<img src="icons/2answer.png" width=115px align="right">](#answer5.2.1)
```

<A name="question5.2.2"></A>
```{block2, type='rmdquestion'}
2. Imagine that legal regulations allow average candy weight in a sample bag to differ 0.10 gram but not 0.25 gram from the weight reported on the sample bag. Our statistical test should pick up (be significant) differences of at least 0.25 gram, but not differences of 0.10 gram or less. How large should our sample be? [<img src="icons/2answer.png" width=115px align="right">](#answer5.2.2)
```
...
A larger sample offers more precision, so the difference between our sample outcome and the hypothesized value is more often sufficient to reject the null hypothesis. For example, we would reject the null hypothesis that average candy weight is 2.8 gram in the population if the average weight in our sample bag is 2.70 gram and our sample is large. But we may not reject this null hypothesis if we have the same outcome in a small sample bag.  
...
```{r sample-size, fig.cap="What is the minimum sample size required for a significant two-sided test result if the sample mean has a particular effect size?", echo=FALSE, out.width="420px", screenshot.opts = list(delay = 5), dev="png"}
...
* Of the three sample outcomes, a sample with 2.40 gram for average candy
weight (red line at the left) is the only sample that is statistically
significant With the initial sample size (N = 15). It is the only weight that
crosses a blue tail, which tells us that it is in the rejection region of the
test. [<img src="icons/2question.png" width=161px align="right">](#question5.2.1)
```
...
statistically significant result for a sample average that differs 0.25 gram
...
signalling differences larger than 0.10 gram and always signalling a difference
of 0.25 gram. [<img src="icons/2question.png" width=161px align="right">](#question5.2.2)
```
...
the sampling distribution that is centred at H1.
...
```{block2, type='rmdquestion'}
4. What is the unstandardized and standardized effect size if average candy weight is 2.83 gram in our sample? [<img src="icons/2answer.png" width=115px align="right">](#answer5.6.4)
```
