...
SPSS version 27 (and later) provides Cohen's d for t tests on one or two means. This is not always the value that we need, so calculate Cohen's d by hand to be on the safe side.
...
* If we hypothesize, for example, that average attitude towards immigrants is at least 6.0 in the population and we find a sample average of 4.5, the unstandardized and standardized effect sizes, then, are the same as in Question 1. 
* In contrast, if our sample average is 7.5, it is covered by the null hypothesis (population average is at least 6.0). In this situation, we could argue that there is no difference between the hypothesized average(s) and the avergae in our sample, so there is no effect. [<img src="icons/2question.png" width=161px align="right">](#question5.2.4)
```
...
* The p values give the (two-sided) probability of drawing a sample with average
