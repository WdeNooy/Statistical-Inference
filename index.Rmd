...
# Chunks with answers to exercises have the echo option set to TRUE, so they are
# always visible. Chunks with answers to Test Your Understanding questions do
# not have thei echo option set, so their visibility is determined by the echo
# option set for all chunks here. Set echo=FALSE at the start of the course and
# set it to TRUE after the last tutorial and before the exam.
...
# Check comments by colleagues in the Tutors group (Hypothes.is) for improvement of the web book.
...
readers$polinterest <- labelled::labelled(readers$polinterest, labels = c(`No interest` = 1, `Very much interested` = 10))
readers$polcynic <- labelled::labelled(readers$polcynic, labels = c(`Not cynical at all` = 1, `Very cynical` = 10))
