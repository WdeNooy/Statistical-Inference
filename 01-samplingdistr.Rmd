...
1. Figure \@ref(fig:random-variable) shows a population of candies. What do you expect the number of yellow candies in a random sample of ten candies from this population to be? Draw several samples and check whether your expectation comes true. [<img src="icons/2answer.png" width=115px align="right">](#answer1.2.1)
```
...
```{block2, type='rmdquestion'}
9. What is the probability that a sample bag of ten candies contains at most three yellow candies if the proportion in the population is .2? [<img src="icons/2answer.png" width=115px align="right">](#answer1.2.9)
```
...
The sample mean is an unbiased estimator of the population mean, so the average weight of all candies in the population (at the factory) is the average of the (average candy weights in the) sampling distribution. And this is the average weight that we expect in a sample drawn from this population (the expected value or expectation). So far, everything is the same as in the case of the number of yellow candies in the sample, which was a discrete random variable because it could take only a limited set of values: 0 to 10.
...
```{block2, type='rmdquestion'}
2. Do you think this is a left-hand probability or a right-hand probability? [<img src="icons/2answer.png" width=115px align="right">](#answer1.3.2)
```
...
If we translate this to the simple candy bag example, we realize that the outcome in our sample does not have to be the true population value. After all, we could very well draw a bag with less or more than twenty per cent yellow candies. 
...
The histogram shows the distribution of candies according to colour in the population. Draw some samples and have a look at the number of yellow candies in each sample as well as the average number of yellow candies over all samples.
