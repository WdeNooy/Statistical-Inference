...
exactly two yellow candies.
```

2. What are all the possible outcomes for the number of yellow candies in our sample? The collection of all possible outcome scores is called the _sampling space_.
...
of yellow candies in the sample.
```
...
are the cases. A sample (bag) contains a particular number of yellow candies.
```
...
bag with ten yellow candies is most unlikely.
```
...
course, and need more samples to arrive at a stable distribution.
```
...
yellow candies is higher (p = 1 - .302 = .698).
```
...
.268 + .302 + .201 = .878. That is a fair chance.
```
...
sample changes in accordance with changes in the population proportion.
```
...
if the sample statistic is the sample proportion. If we would have created the
sampling distribution of the proportion of yellow candies in our sample, the
mean of the sampling distribution would be equal to the proportion of yellow
candies in the population.
...
average value of the sampling distribution of average sample candy weight.
```

2. What do you think, is this a left-hand probability or a right-hand probability?
...
include either the left-hand or right-hand tail of the sampling distribution.
```

4. What is the minimum average weight of the 10% heaviest candy bags?  

```{r eval=FALSE, echo=TRUE}
* Drag the rigth-hand slider handle to the maximum value (6) and then adjust
...
displayed with the left-hand slider is the minimum average candy weight for
these samples.
```
...
distribution is the average of the sampling distribution.
```
...
population mean but it still can be quite different from the population mean.
```
...
1. Figure \@ref(fig:sampling-distribution-summary1) shows a simulated population distribution. What would be the real-world population?
...
ten. This range of values is called the sampling space.
```
...
equals two yellow candies in a sample.
```
...
6. Why, do you think, is the sample characteristic (sample statistic) called a _random variable_?
...
reason for calling it a random variable.
```
...
candies, for example, a factory's stock.
```
...
this example, the average is 2.8 (grams).
```
...
depicted in the blue box within the graph. It is .475.
```
...
grams, that is, 2.90000000000000000000000000(and so on) grams.
```
...
labelled 'probability density' instead of 'probability'.
```
