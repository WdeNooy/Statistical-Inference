...
1. Figure \@ref(fig:sampling-distribution-summary1) shows a simulated population distribution. If we were to do this study in reality, what would be the population?
...
```{r eval=FALSE}
* If twenty per cent of candies in the population
are yellow, we expect about twenty per cent of
...
Most but not all sample statistics are unbiased estimators of the population statistic. Think, for example, of the actual number of yellow candies in the sample. This is certainly not an unbiased estimator of the number of yellow candies in the population. Because the population is so much larger than the sample, the population must contain many more yellow candies than the sample. If we were to estimate the number in the population (the parameter) from the number in the sample---for example, we estimate that there are two yellow candies in the population of all candies because we have two in our sample of ten---we are going to vastly underestimate the number in the population. This estimate is _downward biased_: It is too low.
...
contains the ten per cent samples with largest
...
mean or centre of the sampling distribution
