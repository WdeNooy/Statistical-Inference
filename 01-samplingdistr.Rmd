...
```{r p-values, fig.cap="How do we display p values in a continuous sampling distribution?", echo=FALSE, screenshot.opts = list(delay = 5), dev="png", out.width="420px"}
...
If we translate this to the simple candy bag example, we realize that the outcome in our sample need not be the true population value. After all, we could very well draw a bag with less or more than twenty percent of yellow candies. The regression coefficient, then, can be positive in our sample even if there is no relation or a negative relation in the population. How we decide on this, is discussed in later chapters.  
...
* Values of a sample statistic vary across random samples from the same population. But some values are more probable than other valuess.  
