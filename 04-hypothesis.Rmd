...
```{r eval=FALSE}
1. The null hypothesis is that average media literacy
...
```{r eval=FALSE}
2. The p value of the test is the surface of the tail
...
```{r eval=FALSE}
3. The surface of the blue tails represent the
significance level of the test. Each tail contains
2.5% of the surface, so the significane level is 5%.
The rejection region are the population values under
...
```{r eval=FALSE}
4. If the sample mean (red line) is in the blue tails
...
```{r eval=FALSE}
5. The curve moves horizontally because it represents the
...
```{r eval=FALSE}
6. It is NOT OK to change your null hypothesis when you
...
```{r eval=FALSE}
1. The coloured tails represent the probabilities of
drawing a sample with a (sample) mean that differs a
lot from the (true or hypothesized) population mean.
The (true or hypothesized) population mean is represented
by the center of the distribution.
In this case, the probabilities of the two coloured
tails sum to 5%, so the tails represent the top five
percent of samples that are most different from the
hypothesized or true population mean.
```

2. What is the meaning of t~c~?
```{r eval=FALSE}
2. t~c~ is the value of the test statistic t that
...
```{r eval=FALSE}
3. With a larger sample, we have more information,
so we have more precise results. More sample means
are close to the true population mean. 
In technical terms, the variation of sample means
decreases with larger sample size; sample means are
more alike. The standard deviation of the sampling
distribution is called the standard error. A 
distribution of larger samples has a smaller standard
error.
```
...
```{r eval=FALSE}
4. No, there is not a fixed relation. A particular
average media literacy score, for example, 4.0 on the
bottom scale, does not always correspond to the same
test statistic (t) value on the top scale. Just change
sample size to see this.
The t statistic expresses the difference between the
(true or hypothesized) population value and the sample
value in standard errors. It translates, for example,
the difference between 5.5 (population average of
media literacy) and 4.0 (media literacy in our sample)
into standard errors, for example, 0.5, which yields
a test statstic value of 1.5 / 0.5 = 3.0.
A larger sample yields a smaller standard error. As
a result, we divide the difference in raw scores,
1.5 in the example, by a smaller number, so the
resulting test statistic is larger.
```

5. What is the relation between sample size and critical t values?
```{r eval=FALSE}
5. Smaller samples have slightly larger critical t values.
```
...
```{r eval=FALSE}
1. If the sample mean (red line) is in the blue tails of
...
```{r eval=FALSE}
2. If the sample mean (red line) is between the blue
...
```{r eval=FALSE}
3. The curve moves because it represents the sampling
...
```{r eval=FALSE}
4. A confidence interval contains all (hypothetical)
