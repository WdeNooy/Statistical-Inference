...
* Here, we are only interested in sample averages that are to low, so the
...
```{r eval=FALSE}
* If the null hypothesis changes, the average, hence the center of the
...
knitr::include_app("http://82.196.4.233:3838/apps/sign-two/", height="460px")
```
...
```{r eval=FALSE}
A two-sided significance level of .05 has .025 probability in both the lleft
and right tail of the dstribution. Move the left slider handle to 2.95 and the
...
mean media literacy scores below 2.95 with thos above 8.05.
```
...
4. What is special about a significance level of .05?
