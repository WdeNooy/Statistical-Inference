...
> Key concepts: research hypothesis, statistical null and alternative hypothesis, nil hypothesis, test statistic, p value, conditional probability, significance level (Type I error rate), Type I error, inflated Type I error, capitalization on chance, one-sided and two-sided tests and tests to which this distinction does not apply, rejection region.  
...
knitr::include_url("https://www.youtube.com/embed/xylpuSL9v3I", height = "360px")
```
...
```{r nullsampling, fig.cap="Sampling distribution of average media literacy according to the null hypothesis.", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
...
```{block2, type='rmdquestion'}
4. In the second step of hypothesis testing (click on *Step 2*), we have to formulate a decision rule for rejecting the null hypothesis. The decision rule is customarily based on a significance level of five per cent. In this situation, when do we reject the null hypothesis? And what represents the *rejection region* in Figure \@ref(fig:nullsampling)? [<img src="icons/2answer.png" width=115px align="right">](#answer4.2.4)
```
...
According to our null hypothesis, the population average is 5.5. If average media literacy of children in the population would really be 5.5, which average media literacy scores are most unlikely? We can use a hypothetical sampling distribution with 5.5 as mean value to answer this question. 
...
```{r twosided, fig.cap="Sampling distribution of average media literacy according to the null hypothesis.", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
...
```{block2, type='rmdquestion'}
9. What is the relation between the red number next to the solid red vertical line and the two-sided p value reported in the figure? Pay attention to the dashed red vertical line in your answer. [<img src="icons/2answer.png" width=115px align="right">](#answer4.2.9)
```
...
* We are going to reject the null hypothesis if our sample outcome (average
media literacy) is among the five per cent outcomes that are most unlikely.
The five per cent is the significance level, which is represented by the blue
tails in the figure.
* If average media literacy in our sample is in the blue tails, we reject the
null hypothesis.
* The media literacy scores on the horizontal axis that are under the blue
tails constitute the rejection region. [<img src="icons/2question.png"
...
* A test result is statistically significant if we reject the null hypothesis.
...
* The rejection region (see the previous question), consists of all sample
means below 4.07 and all sample means above 6.93. [<img
src="icons/2question.png" width=161px align="right">](#question4.2.5)
```
...
population.
* Instead of using the blue tails, your can also justify your answer by
...
* The two-sided p value is twice the (one-sided) p value reported as the red number
...
* (left-sided) p Value: The probability of drawing a sample with 3.9 average media
...
```{r sign-left, fig.cap="Sampling distribution of average media literacy.", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
...
```{r onetwosided, fig.cap="Halve a two-sided p value to obtain a one-sided p value, double a one-sided p value to obtain a two-sided p value.", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
...
requires that the sample outcome is on the correct side of the hypothesized
...
equal to the reported  one-sided p value. We have to add the two probabilities
for both tails, so we double the one-sided p value to obtain the two-sided p
value. [<img src="icons/2question.png" width=161px
align="right">](#question4.4.6)
```
...
The exact formula and calculation of a test statistic is not important to us. Just note that the test statistic is usually zero if the sample outcome is equal to the hypothesized population value. In Figure \@ref(fig:crit-df), for example, the t value of a sample with mean 5.5 is zero if the hypothesized population mean is 5.5. The larger the difference between the observed value (sample outcome) and the expected value (hypothesized population value), the more extreme the value of the test statistic, the less likely (lower p value) it is that we draw a sample with the observed outcome or an outcome even more different from the expected value, and, finally, the more likely we are to reject the null hypothesis.
...
Figure \@ref(fig:null-ci) shows media literacy scores in a random sample of children and their average media literacy score (red). The hypothesized average media literacy in the population of children is shown on the bottom axis. The curve represents the sampling distribution if the null hypothesis is true.

```{r null-ci, fig.cap="How does null hypothesis significance relate to confidence intervals?", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
...
Let your statistical software calculate the p value of the test and/or the value of the test statistic. It is important that this step comes after the first three steps. The first three steps should be made without knowledge of the results in the sample (see Section \@ref(cap-chance)).  
...
```{block2, type='rmdimportant'}
"The television station reaches significantly and substantially (_p_ = .61) more than half of all households in Greece in 2012, _z_ = 4.01, _p_ < .001."
```
...
If you have the value of the test statistic but not the p value, you may report the significance level of the test instead of the p value. In this case, you either report "_p_ < .05" if the test is significant at the 5% significance level or "n.s." if the test is <i>n</i>ot <i>s</i>ignificant.  
...
#Create and draw flow chart f or selecting tests in SPSS.
source("flowchart.R")
#Show plot.
p
#Cleanup.
...
```{block2, type='rmdquestion'}
1. Use the data set [households.sav](http://82.196.4.233:3838/data/households.sav) to test the research hypothesis that the TV station does not reach 40 per cent of all households in the population. [<img src="icons/2answer.png" width=115px align="right">](#answer4.8.1)
```
...
the population" into the null hypothesis that the TV station equals 40 per
...
In the RECODE command, the lower limit of a category (e.g., 30,000) equals the upper limit of the preceding category (30,000) instead of one more (30,001). In this way, values between 30,000 and 30,000, for example, 30,000.5, are not skipped. The first category includes all values up to and including 30000. The second category includes all values larger than 30,000, for instance, 30,000.5, up to and including 50,000.
...
Sample size (N = 86) is well over 30, so we need not worry about the (normal) shape of the distribution of the variable in the population.  
...
```{r hypo-testing, fig.cap="Testing null hypotheses.", echo=FALSE, out.width="775px", screenshot.opts = list(delay = 5), dev="png"}
# Use app null-ci.
knitr::include_app("http://82.196.4.233:3838/apps/null-ci/", height="268px")
```
