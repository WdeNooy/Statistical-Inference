...
1. Which statistics are addressed in the examples of research hypotheses above?
...
1. What does the number next to the solid red vertical line represent in Figure \@ref(fig:twosided)?
...
null hypothesis is true. See Question 4.
```

2. What is the relation between the number next to the solid red vertical line and the two-sided p value reported in the figure? Pay attention to the dashed red vertical line in your answer.
...
expected association, so this is an alternative hypothesis.
```
...
used for a two-tailed test at five epr cent significance level.
...
* If the sample mean is falls within the light (or dark-blue) tail at the
...
3. In which situation is a one-sided test statistically significant whereas a two-sided test is not statistically significant at the five per cent significance level?
...
4. In which situation is a one-sided test not statistically significant whereas a two-sided test is statistically significant at the five per cent significance level?
...
value. If it is on the wrong side, it can never be significant in a
...
In the first situation, for example, if we test the null hypothesis that average media literacy is 5.5 in the population, we may rule out the possibility that the population value is higher (lower) than the hypothesized value if we have good reasons to believe that it can only be lower (higher). Another example is a test on a regression coefficient or correlation coefficient. If we assume that exposure to brand advertisements can increase but not decrease brand awareness among consumers, we apply a right-sided test because we are only interested in positive values (larger than the hypothesized zero correlation).

In the second situation, we compare the scores of two groups on a dependent variable. If we compare variation in opinion scores between young and old voters, for example, we may expect that one group can only score higher (more variation: more polarized) than the other group but not the other way around. Again, a one-sided test can be applied.
...
standard error of the sampling distribution (see the preceding chapter). A
distribution of larger samples has a smaller standard error.
```
...
* The p value is .004. Note that we report all results with two decimal places except probabilities, which are reported with three decimals. We are usually interested in small probabilities---less than .05---so we need the third decimal here. If SPSS rounds the p value to .000, report: _p_ < .001.
...
The proportion of households not reached in the sample (p = 0.48) is above
fourty per cent. We have to reject the null hypothesis that the TV station
reaches at most fourty per cent of all households, p = .039 (one-sided).
...
in the population is around 5.5, more specifically, between 4.94 and 5.77. A
test is not statistically significant, t (85) = -0.68, p = .498, 95%CI[4.94;
5.77].
...
The relation between null hypothesis testing and confidence intervals (Section \@ref(null-ci0)) may have given the impression that we can test a range of null hypotheses using just one sample and one confidence interval. For instance, we couldan simultaneously test the null hypotheses that average media literacy among children is 5.5, 4.5, or 3.5. Just check if these values are inside or outside the confidence interval and we are done, right?  
...
hypothesis because the hypothesized value is not zero.
...
3. What part of Figure \@ref(fig:hypo-testing) represents the significance level and rejection region?

```{r eval=FALSE}
* The surface of the blue tails represent the significance level of the test.
Each tail contains 2.5% of the surface, so the significance level is 5%.
...
* We reject the null hypothesis if a test is statistically significant. This means that the probability of drawing a sample with the current or a more extreme outcome (even more inconsistent with the null hypothesis) is below the significance level.
