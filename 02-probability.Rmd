...
the true sampling distribution if the initial
...
categorical variables, such as proportions and
associations. 
```
...
sometimes be quite off the mark. 
```
...
the number of heads that appear in the
...
```{r eval=FALSE}
* There are eight combinations. If the coins are
...
heads in a throw (p = .125), one head (p = .375),
and so on.
```
...
sampling distribution because the sample proportion is an unbiased estimator of
the population proportion. So if we change the population proportion, the
center of the sampling distribution changes accordingly.
* In addition, the sampling distribution becomes less symetrical/more skewed if
the population proportion approaches zero or one. Because proportions cannot be
less than zero or more than one, the sampling distribution cannot remain
symmetrical if the population proportion is near zero or one.
```
