...
size as the initial sample, the new sample must contain all obervations from
...
As you can see, calculating probablities becomes complicated if we sample without replacement. But we can ignore the complications if the population is much larger than the sample because the probabilities hardly differ from the simple probabilities that we have if we sample with replacement. 

In practice, then, we always sample _without_ replacement but our statistical software calculates probabilities as if we sampled _with_ replacement. This is perectly fine as long as the population is much larger than the sample. If the sample contains a large share of the population, however, we should not trust the probabilities that our software reports.
...
the same probability to appear, namely 1/8 = .125. We sum this probability for
