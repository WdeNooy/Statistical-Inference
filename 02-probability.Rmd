...
In contrast, when is it _not_ OK to sample without replacement as we normally do in research? It is not OK to sample without replacement if the population is not much larger than the sample. If the population is not much larger, the probabilities change substantially during the sampling process, so calculating probabilities as if the probabilities do not change would be a problem.
...
1. Download the data set [candies.sav](href="http://82.196.4.233:3838/data/candies.sav) and use SPSS to apply a Fisher-exact test to the association between candy colour and candy stickiness.  
