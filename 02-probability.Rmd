...
```{r models-summary1, echo=FALSE, fig.cap="How do we bootstrap a sampling distribution?", screenshot.opts = list(delay = 10)}
...
need the oroginal population for bootstrapping.
```
...
4. Calculate the exact probability distribution of the number of heads in a toss of three fair coins (Figure \@ref(fig:)).
...
5. In which situations can we use the exact probabilities as approach to the sampling distribution?
...
```{r models-summary2, echo=FALSE, out.width="420px", fig.cap="How do we approximate a sampling distribution with a theoretical probability distribution?", screenshot.opts = list(delay = 10)}
...
```{r bootstrapping, echo=FALSE, fig.cap="How do we create a sampling distribution with bootstrapping?", screenshot.opts = list(delay = 10)}
...
sampling distribution (grey histogram). If there
are much less or many more than ten yellow candies
...
```{r bootstrap-lim, echo=FALSE, fig.cap="How is bootstrapping influenced by sample size?", screenshot.opts = list(delay = 10)}
...
2. How does sample size relate to representativeness of the sample? Twentye percent of the candies in the proportion are yellow.
...
```{r normal-approximation, echo=FALSE, fig.cap="Normal function as theoretical approximation of a sampling distribution.", screenshot.opts = list(delay = 10)}
...
3. Generate some new sampling distirbutions to see if the normal function always yields a good approximation. What changes in the distribution: the mean, the standard deviation, or both?

```{r eval=FALSE}
The mean of the sampling disitrbution does not
change. It is equal to the population mean (mean
candy weight in the population), so the population
mean seems not to be changed when we generate
a new sampling distribution.
The width/peakedness of the sampling distribution
changes. This suggests that the variation (standard
devation) in the population changes if we draw a
new sampling distribution. But regardless of the
amount of variation, the normal curve fits the
...
```{r mean-independent, echo=FALSE, fig.cap="How do we obtain a sampling distribution for the mean difference of to independent samples?", out.width="540px", screenshot.opts = list(delay = 10)}
...
* Samples are independent if, in principle, we can drawn a sample for one group without taking into account the sample for another group of cases. Otherwise, the samples are dependent or paired.