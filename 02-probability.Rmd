...
```{r bootstrapping, fig.cap="A sampling distribution generated with bootstrapping."}
# Variant of app andom-variable (Ch. 1).
# Generate a not too small (so quite representative, N = 50?) random sample from a uniformly distributed population of five colours (don't show the population) ; button to draw one sample with replacement at a time, showing the distribution of colours in the sample (dotplot with coloured dots) and adding the proportion of yellow candies to the histogram of the sampling distribution (y-axis percentage of cases?), which already shows the true sampling distribution (binomial distribution) as a light histogram in the background ; after using this button five times, enable another button to add one thousand samples to the histogram in one go.
```

In Figure \@ref(fig:bootstrapping), draw bootstrap samples to build a sampling distribution of the proportion of yellow candies in a sample. The five colours are equally distributed in the population.
...
3. Does the bootstrap sampling distribution resemble the true sampling distribution? Justify your answer.  
...
```{r bootstrap-lim, fig.cap="Conditions for bootstrapping."}
...
1. Play with sample size. Does the bootstrap sampling distribution always reflect the true sampling distribution?
...
### Bootstrapping with SPSS {#boot-spss}
...
1. <a href="http://82.196.4.233:3838/data/candies.sav" target="_blank"> Download the data set "candies.sav" here</a> and use SPSS to bootstrap the t test on average weight of yellow and red candies (the example above). The test is available in the _Analyze>Compare Means_ menu.
...
```{r normal-approximation, fig.cap="Normal function as theoretical approximation of a sampling distribution."}
...
1. Let the computer draw a lot of samples to generate a sampling distribution of the sample means. Check if the normal function (curve) is a good approximation of the sampling distribution. Pay special attention to the tails because these are used for significance tests (see Chapter \@ref(hypothesis)). Repeat this a few times to see if the normal function always yields a good approximation.
...
### Theoretical approximation in SPSS
