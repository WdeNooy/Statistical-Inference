...
```{r models-summary}
# Interface with three tabs labelled bootstrap/exact/theoretical, containing, (1) app bootstrapping, (2) app exact-approach, (3) app normal-approximation.
```

1. Inspect the app under tab __Bootstrap__. Why does it not show a population? Which type of sampling is better here: with or without replacement. Justify your answers.  

2. Inspect the app under tab __Exact__. Why does this app not show a population? How are the outcome probabilities related to the combinations?  

3. Inspect the app under tab __Theoretical__. Try to explain in your own words why the sampling distribution of a sample mean has a bell shape. Which part of the graph represents the theoretical probability distribution? Ca we always use this theoretical probability distribution?
...
```{r bootstrapping, echo=FALSE, fig.cap="A sampling distribution generated with bootstrapping."}
...
knitr::include_app("http://82.196.4.233:3838/apps/bootstrapping/", height="500px")
```

In Figure \@ref(fig:bootstrapping), an initial sample has been drawn from a population containing five candy colours in equal proportions.  Draw bootstrap samples to build a sampling distribution of the proportion of yellow candies in a sample.

1. How large is the original sample and the bootstrap sample?

2. What is the true sampling distribution in this example? If in doubt, see Figure \@ref(fig:probability-distribution). 
...
If we do allow the same person to be sampled more than once, we sample _with replacement_. The same person can occur more than once in a sample. Bootstrap samples that are sampled with replacement from the original sample can vary because they need not contain all observations in the original sample. Some observations may not be sampled while other observations are sampled several times. You probably have noticed this in Figure \@ref(fig:replacement). Sampling with replacement, we can obtain different bootstrap samples from the the original sample and still have bootstrap samples of the same size as the original sample.  

### Calculating probabilities with replacement  
You may wonder whether it is OK to sample with replacement. The short answer: Yes it is. We usually calculate probabilities as if we sampled with replacement. Suppose we want to calculate the probability sampling two yellow candies from a population in which 20% of the candies are yellow. The probability of drawing two yellow candies is then calculated as .200 * .200: twice the probability of drawing a yellow candy.  
...
So is it OK to sample with replacement? From the point of view of probabilities, it is OK because we usually calculate probabilities as if we sampled with replacement. In practice, however, we never want the same respondent to participate twice in our research because this does not yield new information (unless the respondent cheats).

In contrast, is it _not_ OK to sample without replacement as we normally do in research? It is OK to sample with replacement as long as the population is much larger than the sample. If the population is much larger, the probabilities more or less remain the same during the sampling process, so calculating probabilities as if the probabilities do not change is not a problem.

### Limitations to bootstrapping  

```{r bootstrap-lim, echo=FALSE, fig.cap="Conditions for bootstrapping."}
...
In Figure \@ref(fig:bootstrap-lim), you can adjust the size of the sample drawn from a population of candies equally distributed over five colours. The light histogram in the background represents the true sampling distribution.

1. Play with sample size. Does the bootstrap sampling distribution always reflect the true sampling distribution? When does it do so, when not?

2. How does sample size relate to representativeness of the sample?  
...
```{r bootstrap-dialog, echo=FALSE, out.width= "450px", fig.cap="Screen shot of the SPSS bootstrap dialog."}
knitr::include_graphics("figures/SPSSbootstrap.png")
```
...
```{r exact-approach, fig.cap="Examples of exact aproaches to the sampling distribution."}
...
An exact approach can be used to create a sampling distribution for one proportion such as the proportion of yellow candies in the example above. Exact approaches are also available for the association between two categorical (nominal or ordinal) variables in a cross-tab: Do some combinations of values on the two variables occur relatively frequently? 

For example, are yellow candies more often sticky than red candies? If so, we conclude that color and stickiness are associated. The _Fisher-exact test_ is an example of an exact approach to the sampling distribution of the association between two categorical variables.  
...
If SPSS offers an exact approach of the sampling distribution, the test dialog window contains an _Exact_ button. You will find it in the dialog for cross-tabs (_Analyze>Descriptive Statistics>Crosstabs_) and in several legacy dialogs for non-parametric tests (_Analyze>Nonparametric Tests>Legacy Dialogs_). 
...
![Screen shot of SPSS exact approach dialog](figures/SPSSexact.png)

Are some candy colours more sticky? An exact approach calculates and reports only a p value. We need a cross-tabulation of the two categorical variables and a Fisher-exact test. This test is obtained if both the option _Chi-square_ is selected in the _Statistics..._ dialog and the option _Exact_ is selected in the _Exact..._ dialog. 
...
<iframe width="100%" height="925" src="fisher.htm" frameborder="0" allowfullscreen></iframe>

For the test result, you should interpret the p value reported in the table __Chi-Square Tests__. This value is clearly below .05 so we conclude that the test is statistically significant. It is unlikely that all colours are equally sticky in the population (_p_ = .010). According to the percentages in the cross-tabulation, blue, red, and yellow candies are more often sticky than orange and green candies and the association is strong (Phi = .52). 

In Section \@ref(cond-probdistr), we will learn that we should use the exact p value instead of the default asymptotic p value in a table such as the current one because of the low expected cell values.
...
The crosstab does not show any impossible values for
...
statistically significant, chi-squared (4) = 13.47,
p = .010. 
Yellow and red candies are less often sticky than 
blue, green, and orange candies.
```

2. With the same data, use an exact approach to test if half of the candies are sticky. Hint: Use the binomial test in the _Analyze>Nonparametric Tests>Legacy Dialogs_ menu.  
...
Candy stickiness is a dichotomy as it should be
in a binomial test. There are no further
assumptions.
  
Interpret the results:
  
A binomial test gives us no reason to believe 
that more or less than half of all candies is 
sticky, p = .480.
```
...
```{r normal-approximation, echo=FALSE, fig.cap="Normal function as theoretical approximation of a sampling distribution."}
...
knitr::include_app("http://82.196.4.233:3838/apps/normal-approximation/", height="200px")
```

1. Let the computer generate a sampling distribution of the sample means (in the background, the computer draws a lot of samples). Check if the normal function (curve) is a good approximation of the sampling distribution. 

2. While checking the distribution, pay special attention to the tails because these are used for significance tests (see Chapter \@ref(hypothesis)). The red and green bars represent the 2.5 percent of samples with minimum or maximum average weigth. The vertical lines mark the outer 2.5 percent according to the normal function.

3. Repeat this (Exercise 1) a few times to see if the normal function always yields a good approximation. What changes in the distribution: the mean, the standard deviation, or both?

The normal distribution is a mathematical function linking continuous scores, e.g., a sample statistic such as the average weight in the sample, to p values, that is, to the probability of finding at least or at most this score. Such a function is called a _probability density function_, Section \@ref(cont_random_var).  
...
Note that this rule of thumb uses one minus the probability if the probability is larger than .5. For example, if we want to test the probability of sampling a candy that is not yellow, the probability is .8 and we use 1 - 0.8 = 0.2 in the rule of thumb.  
...
For most of these theoretical probability distributions, sample size is important. The larger the sample, the better. There are additional conditions that must be satisfied such as the distribution of the variable in the population. The rules of thumb are summarized in Table \@ref(tab:thumb). Bootstrapping and exact tests can be used if conditions for theoretical probability distributions have not been met. Special conditions apply to regression analysis (see mediation and moderation).

```{r thumb, echo=FALSE}
knitr::kable(rbind(c("Normal distribution", "proportion", ">= 5 divided by test proportion (<= .5)", " "), c("Normal distribution", "one or two means", "> 100", "OR variable is normally distributed in the population and population standard deviation is known"), c("_t_ distribution", "one or two means", "> 30", "OR variable is normally distributed in each group's population"), c("_t_ distribution", "(Pearson) correlation coefficient", "-", "variables are normally distributed in the population"), c("_t_ distribution", "(Spearman) rank correlation coefficient", "> 30", "-"), c("_t_ distribution", "regression coefficient", "-", "-"), c("_F_ distribution", "3+ means", " all groups are more or less of equal size", "OR all groups have the same population variance"), c("_F_ distribution", "two variances", "-", "no conditions for Levene's _F_ test"), c("chi-squared distribution", "row or cell frequencies", "expected frequency >= 1 and 80% >= 5", "crosstab: 3+ rows or 3+ columns")), col.names = c("Distribution", "Sample statistic", "Minimum sample size", "Other requirements"), caption = "Rules of thumb for using theoretical probability distributions." )
```

### Checking conditions  

```{r check-conditions, fig.cap="Appropriate theoretical distribution and conditions."}
# Select a research question from a list and generate a sample that does or does not meet conditions for using the appropriate theoretical probability distribution: (group) sample size, normal distirbution. Show the research question and display the sample as a frequency distribution (freq_poly) of each single variable (RQ on single mean or correlation) or for each group (RQ on 2 or more group means or group variances) with the number of observations (per group). User selects correct theoretical distribution from a list (normal/t/F) and whether conditions are met (Yes/No). Give feedback (correct answer) to student's choice. Add button to generate a new RQ and sample. 
```

Rules of thumb about sample size are easy to check once we have collected our sample. In contrast, rules of thumb that concern the scores in the population cannot be easily checked because we do not have information on the population. If we already know the population, why would we draw a sample and and do the research in the first place?  
...
knitr::include_app("http://82.196.4.233:3838/apps/mean-independent/", height="660px")
```

1. Press the button once. why are the samples called independent?  

2. Press the button several times. What exactly is the sample statistic in the histogram at the bottom of the app?

3. Press the button to draw one thousand samples. Does the sampling distribution look familiar to you?
...
1. In Figure \@ref(fig:mean-dependent), use the __Sample 1__ button repeatedly to draw a sample with five observations. What is the precise meaning of the numbers on the horizontal axis in the Sample histogram?
...
In one case, SPSS automatically selects an exact approach if the conditions for a theoretical approximation are not met. If you do a chi-squared test to a cross tabulation in SPSS, SPSS will automatically apply Fisher's exact test if the table has two rows and two columns. In all other cases, you have to select a bootstrapping or exact approach yourself if the conditions for a theoretical approximation are not met.
...
* Samples are independent if, in principle, we can drawn a sample for one group without taking into account the sample for another group of observations. Otherwise, the samples are dependent or paired.