...
In this situation, the indirect correlation between political interest and newspaper reading time due to news site use is negative: Positive times negative yields a negative. The indirect correlation tells us that people interested in politics use news sites more frequently and for that reason read less newspapers. The indirect correlation clearly contradicts the regression effect of political interest on newspaper reading time, which is positive: People who are more interested in politics spend more time on reading newspapers.
...
variable tells us if a confounder is a suppressor or reinforcer.
* If the sign (direction) of the indirect correlation between predictor and
oucome due to the confounder (age) is equal to the sign of the correlation
...
The indirect correlation due to age also is negative because age is negatively
...
```{r eval=FALSE, echo=TRUE}
* A (bivariate) correlation in a mediation model can differ from the standardized
...
1. Each variable used as an outcome is numeric. This is a general requirement of a linear regression model. In a path diagram, it means that all mediators and dependent variables must be numeric.
...
# SPSS menu: Regression > PROCESS ; 
# * Model 4, 
# * V3.0: covariates are always predictors for the mediator(s) and for the outcome
...
* Select readingtime as dependent varible (Y).  
...
* PROCESS assumes that covariates predict both the mediator(s) and the dependent variable. You can see this in the reported regression equations: all of them include the covariates as predictors.
...
4. Can the standardized effect of political interest on reading time be larger than 1.0 or smaller than -1.0? If so, in which situation? If not, why not? Use the sliders to check your answer.
...
interest due to age with reading time is negative. Just increase the positive
...
5. How many path models do we need to estimate the model of Figure \@ref(fig:mediation-selftest)?
