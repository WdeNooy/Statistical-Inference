...
If we add a causal order among the predictors of our regression model, we obtain a causal model or path model. Instead of a correlation between two predictors, that is, an undirected association, we now have a directed association, usually called an indirect effect: The first predictor affects the scores on the second predictor, which affects the outcome scores. In this path model, the second predictor mediates the effect of the first predictor on the outcome variable.

We can estimate a path model as a series of regression models. With additional software, we can also estimate the confidence interval and p value of an indirect effect. The causal order underlying the path model, however, is an assumption that we make. A regression model shows the predictive effects, which need not be causal. We cannot proof that the predictive effects are causal. We can only think of arguments that make a causal effect plausible.

### Test your intuition or knowledge {-}

```{r mediation-selftest, fig.cap="How does mediation work and how can we analyze mediation with regression models? The values to the arcs in the diagram are standardized regression coefficients."}
# Use app mediation-commoncause.
```

1. In Figure \@ref(fig:mediation-selftest), which variable is a mediator?

2. What does the curved arc represent? How is its value calculated?

3. How do the numbers tell you that the effect of political interest on newspaper reading time is controlled for age? Hint: Also take into account the correlation values.

4. When is the standardized effect of political interest on newspaper reading time equal to the correlation between these two variables? Use the sliders to check your answer.

5. When does political interest have no (standardized) effect on reading time in this model? Use the sliders to check your answer.

6. Change the correlations in such a way that the partial effect of political interest on reading time is larger than the correlation between the two variables. In this situation, would political interest be a suppressor or reinforcer if it would be excluded from the model?

7. Change the correlations in such a way that the part of the effect of political interest on reading time is spurious if age would be excluded from the model.

8. Can the standardized effect of political interest on reading time be larger than 1.0 or smaller than -1.0? If so, in which situation? If not, why not? Use the sliders to check your answer.
...
```{r mediation-indirectcorrelation, fig.cap="How does the change of a regression coefficient relate to the correlations between, on the one hand, confounder and, on the other hand, the predictor and outcome? The width of lines and arc represent the size of the correlation or regression effect."}
...
### Indirect correlation and confounding size
...
# Adjust app mediation-indirectcorrelation: Add Pol.Cynicism as a fourth confounder (r is -0.18 with political interest and -0.14 with reading time). Colour negative correlations red instead of blue if the associated confouder is selected. This helps to discover that a positive indirect correlation (blue-blue) decreases the partial effect whereas a negative indirect correlation (blue-red) increases it.
```
...
3. Now predict average newspaper reading time by education, political cynicism, news site use, and age. Add the predictors one by one to the regression model in the order secified in the preceding sentence. Does suppression occur here? If so, for which predictor and confounder and in which model(s)?
...
```{r mediation-commoncause, fig.cap="How does a common cause affect regression coefficients? The values in this path diagram represent standardized regression coefficients."}
...
1. In Figure \@ref(fig:mediation-commoncause), which variable is a common cause and which is a mediator?

2. What does the long, curved arc represent? Can you motivate your answer with the values that are linked to the arcs?

3. Why is the standardized effect of political interest on newspaper reading time not always equal to the correlation between these two variables? When is this standardized effect equal to the correlation? Use the sliders to check your answer.

4. When does political interest have no (standardized) effect on reading time in this model? Use the sliders to check your answer.

5. Change the correlations in such a way that the partial effect of political interest on reading time is larger than the correlation between the two variables. In this situation, would political interest be a suppressor or reinforcer if it would be excluded from the model?

6. Can the standardized effect of political interest on reading time be larger than 1.0 or smaller than -1.0? If so, in which situation? If not, why not?
...
```{r mediation-regression, echo=FALSE, fig.cap="Path diagram with unstandardized effect sizes and their 95% confidence intervals.", fig.asp=0.5}
...
3. Which effect sizes with their confidence intervals can we not obtain in this manner?
...
```{r pathdiagram-unstandardized, echo=FALSE, fig.cap="Path diagram with unstandardized effect sizes and their 95% confidence intervals.", fig.asp=0.5}
...
knitr::kable(results, caption = "Bootstrap results for unstandardized indirect effects in a model with two mediators. Effect size (__Effect__), standard error (__Boot SE__), lower and upper levels of the 95% confidence interval (__BootLLCI__ and __BootULCI__).", col.names = c("", "Effect", "Boot SE", "BootLLCI", "BootULCI"), align = c("l", "r", "r", "r", "r")) 
 # Cleanup.
rm(results)
```
...
A path model may yield a lot of direct effects, so it is good practice to present results as a path diagram with the values of the standardized or unstandardized regression coefficients as labels to the arcs. Remember that we must use unstandardized regression coefficients if we have a dummy variable as predictor. A path model conveniently summarizes the results for the reader, especially if (very) weak effects are excluded or de-emphasized (Figure \@ref(fig:results-pathdiagram)). 

```{r results-pathdiagram, echo=FALSE, fig.cap="Standardized direct effects for a path model with one mediator. Small effects are represented by thin arcs."}
# Load and stadardize data.
readers <- haven::read_spss("data/readers.sav")
readers$age <- (readers$age - mean(readers$age))/sd(readers$age)
readers$education <- (readers$education - mean(readers$education))/sd(readers$education)
readers$polinterest <- (readers$polinterest - mean(readers$polinterest))/sd(readers$polinterest)
readers$newssite <- (readers$newssite - mean(readers$newssite))/sd(readers$newssite)
readers$readingtime <- (readers$readingtime - mean(readers$readingtime))/sd(readers$readingtime)
# Create effect sizes.
model_3 <- lm(newssite ~ age + education + polinterest, data = readers)
model_4 <- lm(readingtime ~ age + education + polinterest + newssite, data = readers)
# Create network
...
edges$width <- ifelse(abs(edges$label) > 0.05, 4, 1)
edges$font.color <- ifelse(edges$label >= 0, brewercolors[[5]], brewercolors[[1]])
edges$font.size <- ifelse(abs(edges$label) > 0.05, 45, 30)
...
It is recommended to report standardized coefficients in your path diagram. Standardized regression coefficients are more easily compared. Remember, however, that we don't use standardized regression coefficients if the predictor is dichotomous or a set of dummy variables. 
...
```{r}
# Show how to use SPSS for mediation.

# Add videos explaining how to install and use the PROCESS macro (in the simplest mediation model with one mediator and no moderator, Model 4 and Model 6 with two mediators but no moderators ; show how covariates can be added with the choice to use them as predictors for the mediator(s) and/or the outcome). Recommend to use PROCESS if there is only one mediator or two  serial (consecutive) mediators. Only use SPSS if there are (two or more) parallel mediators or more than two serial mediators. Note: don't paste PROCESS commands!

# {note: use completely standardized indirect effect in PROCESS}
```
