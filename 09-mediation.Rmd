...
2. Try other combinations of Age and a second independent variable. When does the regression coefficient of Age change markedly? Can you explain why it changes as it does?
...
site use as an independent variable to the model.
```

3. Interpret the meaning of the simple regression models and explain why they do not change if independent variables are added. In which situation are the light and dark results equal?
...
no longer a confounder if it is included in the model.
```
...
Interest in sports as a predictor in the regression model.
```
...
2. In the series of models that you estimated in Exercise 1, for which predictors is age a suppressor or reinforcer?
...
1. Why is political interest called a mediator in Figure \@ref(fig:mediation-commoncause)?
...
* A mediator is also called an intermediary or intervening variable.
```

2. In Figure \@ref(fig:mediation-commoncause), what does the long, curved arrow represent? Can you motivate your answer with the values that are linked to the arrows?
...
3. Which effect sizes with their confidence intervals are not reported in regression models executed with SPSS?
...
3. Add interest in politics as a covariate to the model of Exercise 2. Does PROCESS use the covariate as predictor for the mediator (news site use), for the dependent variable (newspaper reading time), or both? Present the unstandardized effects as a path diagram and add the indirect effect to it.
...
3. When is the correlation between political interest and reading time completely spurious in this model? Use the sliders to check your answer.
...
4. How many path models do we need to estimate the model of Figure \@ref(fig:mediation-selftest)?
...
Extra. The standardized effect of political interest on reading time can be larger than 1.0 or smaller than -1.0. In which situation? Use the sliders to check your answer.
