...
### Partial effect  
Conceptually, the regression model first removes the variation in the outcome that is predicted by all other predictors. Then it determines how well the reaming variable predicts the variation that is left (residual variation). This is the variation in outcome scores than can be predicted by this particular variable but not by any of the other predictors in the model.
...
1. The file <a href="http://82.196.4.233:3838/data/readers.sav" target="_blank">readers.sav</a> contains the (simulated) data on newspaper reading time that we have used as example in this chapter. Analyze @@
...
Note that the indirect effect is interpreted in terms of the measurement units of the initial predictor (age in tens of years) and the final outcome (reading time): a difference in years predicts a difference in reading time. 
...
The total effect is the regression coefficient that you would get if you regress the outcome only on this predictor (a simple regression model). In the current example, the total effect of age on newspaper reading time is larger than the direct effect. The total effect is partly spurious (see Section \@ref(spuriousness)). The total effect can also be smaller than the direct effect, namely, if there are suppressors (Section \@ref(suppression)).
...
We analyse a path model as a series of regression models, so the general rules for reporting mediation are the same as for reporting regression analyses (see Section \@ref(reportmoderation)). Report:

1. Unstandardized or standardized regression coefficients for all predictors.
...
results <- rbind(
  ...
  c("$R^2$ = 0.93, $F$ (3, 308) = 617.40, $p$ < .001", "", "", "", ""),
  ...
  c("$R^2$ = 0.89, $F$ (4, 307) = 290.85, $p$ < .001", "", "", "", ""),
...
A path model may yield a lot of direct effects, so it is good practice to present results as a path diagram with the values of the unstandardized or standardized regression coefficients as labels to the arcs. This conveniently summarizes the results for the reader, especially if (very) weak effects are excluded or de-emphasized (Figure \@ref(fig:results-pathdiagram)). Report all numerical results in a table, such as Table \@ref(tab:mediation-table), which can be placed in an appendix.
...
```{r}
# Add videos explaining how to install and use the PROCESS macro (in the simplest mediation model with one mediator and no moderator, Model 4 and Model 6 with two mediators but no moderators ; show how covariates can be added with the choice to use them as predictors for the mediator(s) and/or the outcome)

# {note: use standardized regression coefficients (multiply for indirect effect = completely standardized indirect effect in PROCESS)}
```

### Exercises

1. Use <a href="http://82.196.4.233:3838/data/readers.sav" target="_blank">readers.sav</a> to analyze the causal model, as depicted in Figure \@ref(fig:pathdiagram-unstandardized), with a series of regression models in SPSS. Draw a path diagram to present the standardized results and interpret their sizes and statistical significance.
...
2. To what extent is the effect of age on newspaper reading time mediated by news site use? Use the data of Exercise 1 and PROCESS to estimate both the unstandardized and standardized indirect effect in a model containing only these three variables.
...
3. Add interest in politics as a covariate to the model of Exercise 2. Are you going to use it as a covariate for the mediator (political interest), the outcome (newspaper reading time), or both? Motivate your choice. Present the unstandardized effects as a path diagram and add the indirect effect to it. Which direct effects are not present in this model?
...
4. Change the model of Exercise 3 so political interest is now a second mediator in addition to news site use. Estimate the model with PROCESS. Report the indirect effects as a table and and interpret them. 
...
5. {TBD: similar exercises for other data sets}
