...
narrower the interval estimate: precision is higher. * The higher our
confidence level, the more population values we include, the broader the
interval estimate: precision is smaller.
```
...
```{r eval=FALSE}
* The sampling distribution becomes more peaked/less flat with a larger sample
because more sample means are found near the (true) population mean.
```
