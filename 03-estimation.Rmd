...
```{r, eval=FALSE}
* The average of a distribution of sample means is
...
```{r, eval=FALSE}
* The probability to draw a sample (bag) with a sample mean
...
```{r, eval=FALSE}
* Raising the confidence level increases the width of the
...
```{r, eval=FALSE}
* A larger sample size makes the sampling distribution more
...
```{r, eval=FALSE}
* Increasing sample size decreases the standard error, so
...
```{r, eval=FALSE}
* A confidence interval always belongs to a population statistic 
...
```{r eval=FALSE}
* A sample mean is an unbiased estimator of the population
...
```{r eval=FALSE}
* The exact value of the upper (right) limit changes from
...
```{r eval=FALSE}
* The more population values we exclude, the larger the 
...
```{r eval=FALSE}
* Larger samples offer more information, so they allow a more
...
```{r eval=FALSE}
* The sampling distribution because more peaked, less flat 
...
legend).
* It gives an impression of how different
...
```{r eval=FALSE}
* The axis at the bottom gives average candy weight in grams
...
above the mean (3.6 gram). 
* The difference is 0.8 gram, so one
...
```{r eval=FALSE}
* If we increase the standard error, one standard error
...
```{r eval=FALSE}
* In a normal distribution, z is the test statistic.
...
```{r eval=FALSE}
* The interval estimate shows the width of the interval
...
```{r eval=FALSE}
* Using the answer to Question 3, we may use the
...
We are 95 percent confident that average candy
