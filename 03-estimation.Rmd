...
```{r interval-level, fig.cap="Factors that affect the precision of an interval estimate."}
...
Figure \@ref(fig:interval-level) shows a sampling distribution of average candy weight in candy sample bags. The horizontal arrow indicates the precision of the interval estimate.

1. How, do you think, does the precision of the interval estimate change if you change the confidence level? First write down what you expect and why you expect that. Then check what happens if you change the confidence level slider.
...
```{r se-point-est, fig.cap="The standard error: How wrong are point estimates?"}
...
Figure \@ref(se-point-est) shows both a sample of candies and the (eventual) sampling distribution. A horizontal arrow in the sample displays the difference between the sample mean and the true (population) mean, which is 2.8 gram. The horizontal arrows in the sampling distribution show the average difference between the population mean and the average mean in all samples that have been drawn. 

1. Explain why we can interpret the horizontal arrows as error.  

2. Use the button __Add Sample__ several times. Explain why the horizontal arrow in the sampling distribution expresses variation, for example, standard deviation. 

3. Use the button __Add 10 Samples__ several times. What happens to the length of the horizontal arrow in the sampling distribution? 

4. If we would add the means of very many samples to the sampling distribution, what would the horizontal arrow mean here?

The word _error_ reminds us that the standard error tells us the size of the error that we are likely to make (on average under many repetitions) if we use the value of the sample statistic as a point estimate for the population value. Let us assume, for example, that the standard error of the average weight of candies in a sample bag is 0.2. Loosely stated, this means that the average difference between the true average candy weight and the average candy weight in a sample is 0.2 if we draw very many samples from the same population. 
