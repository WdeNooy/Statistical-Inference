...
```{r, eval=FALSE}
1. The probability to draw a sample (bag) with a sample mean 
(average candy weight) between the value of the left (lower) limit
and the right (upper) limit.
```
...
```{r, eval=FALSE}
2. Raising the confidence level increases the width of the interval
because it increases the area under the curve between the interval limits.
This makes sense because the confidence level is the probability of drawing
a sample with a mean (sample statistic value) within the interval. If we want
to have a higher probability, we must be mor einclusive, that is, we must 
allow for a greater range of sample means.
```

3. What happens to the graph if you change sample size?
```{r, eval=FALSE}
3. A larger sample size makes the sampling distribution more peaked,
so the interval containing the middle 95% of all samples becomes narrower.
...
```{r, eval=FALSE}
4. Increasing sample size decreases the standard error, so the two
are negatively correlated. A larger sample size creates a more peaked
distribution, which indicates lower variation (scores are closer to 
the mean), standard error expresses the variation in the sampling distribution.
Actually, it is the standard deviation of the sampling distribution. This 
makes sense: A larger sample contains more information, so its mean should 
(on average) be closer to the true population mean (hence to the mean of 
...
```{r, eval=FALSE}
5. Nothing changes. Critical values are fixed values in terms of standard 
