...
```{r estimation, fig.cap="Point and interval estimates, confidence intervals."}
# Combination of apps interval-level, interval-size, and crit-values: A normal curve (M = 2.8, SE = 0.1 (SD population = 0.5 and sample size = 25)) as sampling distribution with two vertical lines marking interval limits (initially set at 2.5%/95%/2.5%) ; percentages indicating the area under the curve within and outside the limits ; double arrow from mean to interval limits indicating the interval estimate ; double x-axis: the first in grams and the second in standard errors, both axes labelled ; 3 sliders: confidence level, sample size, and standard error ; any slider change will change the position of the interval limits (to represent the selected confidence level) ; changing confidence level also changes the percentages ; changing sample size or standard error also changes the scale of the x-axis in standard errors ; if possible, changing sample size (or standard error) should also change the slider position of the standard error (or sample size) to show the relation between the two.
```
...
1. The percentage in between the two vertical lines can be interpreted as a probability. A probability of what? 
```{r, echo=FALSE}
#1. The probability to draw a sample (bag) with a sample mean (average candy weight) between the value of the left (lower) limit and the right (upper) limit.
```
2. The double arrow represents an interval of sample means, in this example average candy weight. What happens if you change the confidence level? Explain why this makes sense.
```{r, echo=FALSE}
#2. Raising the confidence level increases the width of the interval because it increases the area under the curve between the interval limits. This makes sense because the confidence level is the probability of drawing a sample with a mean (sample statistic value) within the interval. If we want to have a higher probability, we must be mor einclusive, that is, we must allow for a greater range of sample means.
```
3. What happens to the graph if you change sample size?
```{r, echo=FALSE}
#3. A larger sample size makes the sampling distribution more peaked, so the interval containing the middle 95% of all samples becomes narrower. We have to expect less variation in sample means, so we have a more precise estimate of the sample mean that we are very likely (95% probability) to draw.
```
4. What happens to the standard error slider if you change sample size? How are sample size and standard error linked? What characteristic of the sampling distribution is expressed by the standard error?
```{r, echo=FALSE}
#4. Increasing sample size decreases the standard error, so the two are negatively correlated. A larger sample size creates a more peaked distribution, which indicates lower variation (scores are closer to the mean), standard error expresses the variation in the sampling distribution. Actually, it is the standard deviation of the sampling distribution. This makes sense: A larger sample contains more information, so its mean should (on average) be closer to the true population mean (hence to the mean of the sampling distribution).
```
5. The values of the interval limits---average candy weights in this example---on the scale in standard errors are called critical values. What happens to the critical values if you change sample size?
```{r, echo=FALSE}
#5. Nothing changes. Critical values are fixed values in terms of standard errors if the confidence level and the degrees of freedom do not change. If this was a t distribution, the critical values would slightly decrease with larger sample size but this change is too small to be of practical relevance, so it is ignored in this booklet.
```
6. What happens to the critical values if you change the standard error?
```{r, echo=FALSE}
#6. Nothing changes. Critical values are fixed values in terms of standard errors if the confidence level and the degrees of freedom do not change.
```
7. What happens to the critical values if you change confidence level?
```{r, echo=FALSE}
#7. Higher confidence levels yield more extreme critical values because the interval must cover a larger prportion of all possible samples.
```
...
```{r, echo=FALSE}
#8. A confidence interval always belongs to a population statistic (parameter). In this example, we use the sample mean to estimate the populaiton mean, so the confidence interval refers to average candy weight in the population. The point estimate, which gives a single number instead of an interval, would be 2.8 grams here.
```
...
```{r ci-borders, fig.cap="Sampling distribution and interval estimate."}
...
1. Move the slider until you have found the interval containing 95% of all samples, which are closest to the (true) population value. Press the button to repeat with a different sampling distribution.  

2. What is average candy weight in the population of candies?  
...
```{r interval-level, echo=FALSE, fig.cap="How does the confidence level affect the precision of an interval estimate?", out.width="420px", screenshot.opts = list(delay = 10)}
...
```{r eval=FALSE}
The more population values we exclude, the larger the probability that the true population value is not in our estimated interval. In other words, our confidence is smaller if increase the precision of our estimate or, in other words, if we reduce the width of the estimated interval.
```

2. What happens if we want to be 100% sure that our interval contains the true population value?

```{r eval=FALSE}
If we want to be 100% sure, we cannot rule out any possible average weight as the true population value. As a consequence, the interval becomes infinitely wide. Of course, such an interval is completely useless because it tells us that any value is possible.
```
...
```{r interval-size, fig.cap="Factors that affect the precision of an interval estimate."}
...
1. How, do you think, does the precision of the interval estimate change if you change the size of the sample? First write down what you expect and why you expect that. Then check what happens if you change the confidence level slider.
...
```{r se-point-est, echo=FALSE, fig.cap="The standard error: How wrong are point estimates?", out.width="420px", screenshot.opts = list(delay = 10)}
...
```{r crit-values, fig.cap="Critical values and the normal distribution."}
# same as interval-size but the slider adjusts the standard error (start value = 0.1, so one standard error aligns with 0.1 gram above or below average; slider range 0.05 and 0.15?) ; the vertical lines are fixed at 2.5% and 97.5% of the cumulative area under the normal curve ; first x-axis has fixed scale in grams (population mean is 2.8) ; add a second x-axis representing z scores with values -1.96, -1.0, 0, 1.0, 1.96 ; if the standard error is adjusted, the normal curve changes as well as the scale of the second x-axis and the two verticl lines at -1.96 and 1.96. 
```
...
```{r pop-means-ci, fig.cap="Population means for which the sample mean is within the interval estimate."}
...
Figure \@ref(fig:pop-means-ci) shows the sample mean (lower scale) and a range of population means (upper scale), click somewhere on the upper scale to select a possible value for the population mean. The app will then show the interval estimate for the sample mean if this is the true population value. If the true sample mean is within this interval, the population mean is represented by a green dot. Otherwise, a red dot marks the population mean. The app will also show the _z_ value of the true sample mean if the selected population mean would be the true population mean.

1. Click repeatedly on the upper line (population) to find the highest and lowest value of the population mean for which the sample mean is in the interval of sample means that have 95% probability to occur. Try to minimize the number of clicks you need. 

2. How does the depicted interval estimate help you to minimize the number of clicks you need?

3. How does the _z_ values of the sample mean help you to minimize the number of clicks you need?
...
<table><tr><td>Jerzy Neyman introduced the concept of a confidence interval in 1937:
...
### Instruction
SPSS usually displays the 95% confidence interval automatically if you use the standard-normal (_z_) or _t_ distribution. For an example, see the independent t test output in Figure \@ref(fig:bootstrap-indep). This figure also shows that bootstrapping also yields confidence intervals. If you want another confidence level than 95%, you have to change this in the _Options..._  or _Statistics..._ dialog. Figure \@ref(fig:dialog-conflevel) shows the _Options..._ dialog to a one sample t test.

```{r dialog-conflevel, out.width="280px", fig.cap="SPSS dialog to change the confidence level."}
knitr::include_graphics("/figures/SPSSconflevel.png")
```

Regression analysis is an exception (command _Analyze > Regression > Linear_). Here, you have to select the option _Confidence intervals_ in the _Statistics..._ dialog to obtain confidence intervals for the regression coefficients.
...
4. Use SPSS to determine if candy colourfulness after exposure to sunlight (colour_post) depends on candy weight and candy sweetness. Interpret the 95% confidence intervals for both effects. 
    Hint: If you forgot which analysis technique you need to answer this question, use the flow chart in \@ref(@@).
