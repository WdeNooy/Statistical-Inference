...
1. The grey line in Figure \@ref(fig:mediation-multipleregression) represents the effect of interest in politics on newspaper reading time if interest in politics is the only independent variable. Check the box for age to add it as a second independent variable. What happens to the regression lines and the standardized regression coefficient (_b*_)? [<img src="icons/2answer.png" width=115px align="right">](#answer9.1.1)
```
...
```{block2, type='rmdquestion'}
3. If we want to know the effect of political interest on newspaper reading time that does not include the effect of other variables, which model should we use: the simple model with political interest as only predictor, a model in which we control for one additional predictor, or a model controlling for all additional predictors? [<img src="icons/2answer.png" width=115px align="right">](#answer9.1.3)
```
...
* If we add age as a second predictor, the effect of interest in politics changes: 
    ...
    + the standardized regression coefficient (_b*_) drops from 0.14 for the (simple) model without age to 0.04 for the model including age as independent variable;
...
* If we only add education to the model, the blue line is steeper than the grey line. The standardized regression coefficient (_b*_) becomes stronger; it changes from 0.14 to 0.19. The partial effect of political interest on newspaper reading time becomes stronger if we control for education.
* This also happens if we only add news site use to the model. The standardized regression coefficient (_b*_) increases from 0.14 to 0.15. This increase is too small to see a clear difference between the blue and grey regression lines. [<img src="icons/2question.png" width=161px align="right">](#question9.1.2)
```
...
* If we include both political interest and age as independent variables, political interest can only predict variation in reading time among participants that cannot be predicted by age. The effect of political interest is cleansed from any effects of age that may have been included in the effect of political interest that we obtained in the simple regression model with political interest as only predictor.
* Every additional independent variable that we control for by adding it to the regression model removes the effect of that variable that may otherwise be captured by the effect of political interest. The more variables we control for, the more the regression coefficient for political interest reflects the effect of political interest instead of effects of other variables.
* Therefore, a model controlling for all additional predictors is the best model to know the effect of political interest on newspaper reading time that does not include the effect of other variables. [<img src="icons/2question.png" width=161px align="right">](#question9.1.3)
```
...
Note that we do NOT use _indirect correlation_ to refer to negative correlations in this book.
...
* Note that we compare the signs of the correlations of the confounder with the sign of the predictor in the model without the confounder. A variable is no longer a confounder if it is included in the model. [<img src="icons/2question.png" width=161px align="right">](#question9.3.1)
```
