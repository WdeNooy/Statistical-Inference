...
Instead of presenting the numerical results in the text, we may summarize them in an APA6 style table, such as Table \@ref(tab:report-moderation-table). Note that _t_ and _p_ values are not reported in this table, the focus is on the confidence intervals. The significance level is indicated by stars.
...
Note: Age was mean-centered for all 87 cases but the regression model only uses the 85 cases without a missing value on any of the variables. As a result, the mean of age scores in the regression model is not exactly zero. We can still use zero here because we merely want to refer to an age value that is in the center of the distribution. Both zero and nearly zero are in the center. Also due to the missing values, the standard deviation of age reported in the regression analysis is slightly different (1.937) from the standard deviation of this variable provided by SPSS Frequencies (1.990). We use the standard deviation reported in the regression analysis.
