...
When we evaluate a p value, we had better take into account the probability that we reject the null hypothesis, which is test power. If test power is low, as it often is in social scientific research without very large samples, we should realize that there can be a substantive difference between true and hypothesized population values even if the test is not statistically significant.
...
* Let us follow the steps outlined in the question:
    - If we choose a small sample size of 20 candies, we have to set the true population mean to at least 3.1 to obtain test power that meets the minimum requirement of .80. For all population values between 2.8 and 3.1, we have insufficient test power, so the chance of drawing a sample that rejects the null hypothesis is too low. We expect (too many) non-significant test results for population values between 2.8 and 3.1, so these population values are plausible if the test result turns out to be not statistically significant.
...
```{block2, type='rmdquestion'}
4. When is a large effect in the population more likely: with a statistically significant effect in a small sample or in a large sample? Compare situations with at least .80 test power. (Press _Take 5 new samples_ after you adjust a slider to see the change.) [<img src="icons/2answer.png" width=115px align="right">](#answer6.4.4)
```
...
* With sample size 10, the population mean must be set to 3.4 for .80 test power. (Press _Take 5 new samples_ after you adjust the population mean slider to see this.)
* With sample size 100, a population mean of 3.0 already yields test power (well) above .80.
* With the larger sample (_N_ = 100), we expect a statistically significant result for a population value of 3.0, that is, for an effect size of 3.0 - 2.8 = 0.2. At this population value, test power is sufficiently high to expect that the null hypothesis is rejected. 
