...
* The null hypothesis for the main effect of endorser is that subjects exposed
...
differences between subjects' willingness to donate scores in the sample can
...
the left-most orange dot represents the willigness score of a subject who was
exposed to Clooney as endorser. The orange line represents the average
willingness score of the subjects who were exposed to Clooney. The dotted red
...
score, for example, the average willingness score of all subjects who were
exposed to Clooney, and the average score of all subjects (the grand or
overall mean).
```
...
willingness scores and the average willingness scores of all subjects. Square
...
subjects, which is less than ten percent of the smallest group (N = 45).
...
In the preceding section, we have looked at the effect of a single factor on willingness to donate, namely, the endorser to whom subjects are exposed. Thus, we take into account two variables: one predictor and one outcome variable. This is an example of a bivariate analysis.
...
* If you ensure that the average score for Clooney, Jolie, and subjects not
...
interaction effect of endorser and sex of the subject, F (2, 137) = 4.63, p =
...
the table with between-subjects effects instead of reporting all F test
results in the text.
