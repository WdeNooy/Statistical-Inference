...
the group in the middle moves closer to the top group (or the bottom group).
```
...
Square and sum them to get the overall variance. Take the square root of the
...
the three endorsers, F (2, 140) = 7.44, p = .001.  
...
smokers (M = 0.80, SD = 1.67). Average attitude towards smoking scores are
  significantly different, F (2, 82) = 18.85, p < .001. We have sufficient
evidence to conclude that one group has a more positive attitude towards
smoking than another group in the population of adults.
```
...
* The test result for every effect, consisting of the test name (_F_), the degrees of freedom, and the significance (p value). APA6 prescribes the following format if you report the test result within your text: F (df1, df2) = F value, p = p value. Note that df1 is the degrees of freedom of the factor and df is the degrees of freedom of the error (within-groups).
...
.011. We may conclude that the effect of the endorsing celebrity on an  
