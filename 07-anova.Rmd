...
1. In the sample of (12) participants displayed as dots in Figure \@ref(fig:anova-means), what do the double-sided vertical arrows represent?
...
dotted black arrows are not clearly related to eta-squared.
```
...
```{r anova-posthoc, fig.cap="Which groups have different average outcome scores in the population? The p values belong to independent t tests on a the means of two groups.", echo=FALSE, screenshot.opts = list(delay = 5), dev="png", out.width="525px"}
...
* If the lines are not parallel, one of two situations occurs:
 ...
 females than for males (or the other way around). The lines connecting the 
 means for females are steeper than those for males.
 ...
 donate than females if they see another endorser. The lines connecting the 
 means for females decrease while those for males increase or the other way
 around.
```
...
1. Use the data in [donors.sav](http://82.196.4.233:3838/data/donors.sav) to test if the effect of celebrity endorsement on adults' willingness to donate is different for females than for males. Check the assumptions and interpret the results. Create a plot to communicate your results.
...
interval for the differences between subgroups.
```

2. Use the same data as in Exercise 1 to test if the effect of celebrity endorsement on willingness to donate depends on remembering the campaign. Check the assumptions and interpret the results. Again, create a plot to communicate your results.
