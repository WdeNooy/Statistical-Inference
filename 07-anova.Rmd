...
```{r anova-overview}
# Use app anova-interaction. Manipulate group means in a a means plotsof a 3x2 design. Create main and interaction effects and inspect effect size and statistical significance.
```
...
3. What is the null hypothesis of the F test reported in Figure \@ref(fig:anova-overview)?

4. How should we interpret eta^2^ in Figure \@ref(fig:anova-overview)?
...
library(ggplot2)
ggplot(d, aes(endorser, willingness_av)) + geom_point() + geom_line(aes(group = const)) + theme_classic() + scale_y_continuous(limits = c(1, 10), breaks = c(1, 5, 10)) + labs(x = "Endorser", y = "Average willingness to donate")
rm(d)
...
Playing with the group means in Figure \@ref(fig:anova-between), you may have noticed that eta^2^ is high if there are large differences between group means. In this situation we have high between groups variance, so we can predict a lot of the variation in outcome scores between subjects. 
...
```{r anova-within, fig.cap="How does within group variance reate to between groups variance?"}
...
Applying one-way analysis of variance in SPSS and interpreting the results is explained in Section \@ref(comp-means).
...
# Similar to app anova-between but with a duble classification of observations (according to endorser and sex).
...
```{r anova-moderation, fig.cap="How can we recognize main effects and moderation in a means plot?"}
...
5. Adjust the means in such a way that the effect of endorser is the opposite for males than it is for females.

6. Adjust the means in such a way that there is no main effect of endorser but still the effect is moderated: there are effects for males and for females.
...
library(ggplot2)
ggplot(d, aes(endorser, willingness_av, colour = sex)) + geom_point() + geom_line(aes(group = sex)) + theme_classic() + scale_y_continuous(limits = c(1, 10), breaks = c(1, 5, 10)) + labs(x = "Endorser", y = "Average willingness to donate")
rm(d)
```
...
```{r anova-interaction, fig.cap="How can we recognize main effects and moderation in a means plot?"}
# Goals: Recognize effect size and statistical significance of interaction effects.
# Exactly the same app as anova-moderatio but now add Eta^2 and the F value plus p value of the interaction effect.
# Initial means.
d <- data.frame(endorser = factor(c("Nobody","Clooney","Jolie","Nobody","Clooney","Jolie"), levels = c("Nobody","Clooney","Jolie")), sex = as.factor(c(rep("male", 3), rep("female", 3))), willingness_av = c(3, 5, 7, 4.5, 6.5, 8.5))
```
...
```{r 2way-meansplot, echo=FALSE, fig.cap="An example of a means plot."}
# {TBD}
...
library(ggplot2)
ggplot(d, aes(x = endorser, y = mean_willing, group = sex, color = sex)) + geom_path() + geom_point() + theme_general() + xlab("Endorser") + ylab("Average willingnss to donate") + scale_y_continuous(limits = c(1, 10), breaks = seq(1, 10))
# Cleanup.
rm(d)
```
...
* For each effect worth interpretation, because it is sizable and/or statistically significant, report eta-squared (eta^2 or  η2) and interpret it in terms of effect size. If you have to calculate eta-squared by hand, divide the between-groups sum of squares of an effect by the total sum of squares (SPSS: corrected total).
...
* As always, don't forget to mention the units (observations) and the meaning of the variables (factors and outcome). They describe the topic of the analysis.
...
```{r SPSS-2way, echo=FALSE, eval=FALSE}
# TBD: videos to executure two-way analysis of variance in SPSS with Analyze > General Linear Model > Univariate
# Uit werkboek IS: 
...
#     In tegenstelling tot Van Peet et al. gebruiken we de partiële η2 niet als maat voor de effectgrootte. Vink dus de optie Estimates of effect size NIET aan. Deze maat is omstreden: een hoofdeffect heeft in een eenwegs-variantieanalyse bijvoorbeeld een andere partiële η2 dan in een tweewegs-variantieanalyse ook al blijft de tussengroepen en totale kwadratensom exact gelijk. In plaats daarvan gebruiken we de gewone η2, die je echter met de hand moet uitrekenen.
...
The test on homogeneity of variances, however, is not
statistically significant, F (8, 76) = 1.79, p = .092.
We are not sufficiently confident to conclude that the
variance of smoking attitude in the population is 
unequal for all subgroups but the p value must make us 
doubt that they are equal. We may not meet the 
assumptions.
...
Exposure clearly matters to smoking attitude according to 
our two-way analysis of variance. High exposure is associated
with a more negative attitude towards smoking (M = 2.99,
SD = 0.50) than medium exposure (M = 3.78, SD = 0.53, 
difference: p < .001 ) and low exposure (M = 3.95, 
SD = 0.62, difference: p < .001). There is no remarkable 
difference in attitude between adults with low or
medium exposure (difference p = .639).
Smoking status does not seem to be related to 
attitude towards smoking by itself. There is, however,
a very weak (eta2 = 0.07) but statistically non-significant 
interaction effect of smoking status with exposure on
attitude towards smoking.
If we inspect the means plot, we see that the effect
of exposure on attitude is moderated by smoking status.
Former smokers tend to be more negative about smoking
if they have medium exposure to the campaign than 
