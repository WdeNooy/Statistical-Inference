...
```{r eval=FALSE}
* No, the F test in analysis of variance does not tell us which groups have
significantly different average population outcome scores.
* Just switch the means of two groups. You will get the same F test results as
before, but now different pairs of groups have different means.
```

2. Is it possible that the F test is not statistically significant but at least one of the t tests that compare groups one by one (p values to vertical arrows) is statistically significant? Check your answer with Figure \@ref(fig:anova-posthoc).

```{r eval=FALSE}
* Yes, this is possible. For example, enter the following group means: Clooney
6, Jolie 7, and No endorser 5.
* With three t tests, the probability that at least one t test is statistically
significant, is much larger than .05. Therefore, we are likely to find a
statistically significant t test even if the F test is not statistically
significant.
```

3. Is it okay that we apply both an F test and several t tests to the same data on group differences?

```{r eval=FALSE}
* No, this is not okay. Every additinal significance test increases the
probability that we find a statistically significant result even if there is no
difference between the group means in the population.
```
