...
```{r eval=FALSE}
* Moderation means different differences. This is best seen in the plot. The
green arrows represent the differences between average willingness of males and
females for one endorser. If the green arrows are not equally long or point in
opposite directions, the differences between females and males are not the
same for all endorsers.
...
* Now, sex moderates the effect of endorser on willingness or, equivalently,
endorser moderates the effect of sex on willingness.
```
...
```{r eval=FALSE}
* The double-sided vertical arrows represent the average willingness to donate
for the three experimental conditions: exposure to Clooney as endorser, Jolie
as endorser, or no celebrity endorser.
```
...
the group in the middle moves closer to the top group (or the botto group).
```
...
willingness scores and the average willingness scores of all participants. Square
them to get the overall variance (or standard devation).
```
...
between scores and their group means) remain the same (but note that the
vertical scale of the graph may change, so the red arrows may seem to increase
or decrease a little).
...
Situations where samples cannot be regarded as independent are the same as in the case of t tests (see Section \@ref(dependentsamples)). For example, samples of first and second observations in a repeated measurement design should not be regarded as independent samples. Some analysis of variance models can handle repeated measurements but we do ot discuss them here.
...
2. The data set [smokers.sav](http://82.196.4.233:3838/data/smokers.sav) contains (simulated) information on smoking behaviour and attitude towards smoking for a random sample of adults. Does the attitude towards smoking differ among smokers, former smokers, and non-smokers (variable: *status3*)?
...
The Levene test is not staistically significant, F (2, 82) = 0.52, p = .595,  
so we may assume that smoking attitude for the three groups have equal  
population variances.  
  
Interpret the results:  
  
Average attitude towards smoking scores are not significantly different, F (2,  
82) = 1.04, p = .358. Therefore, we have no reason to conclude that one group  
has a more positive attitude towards smoking than another group in the  
...
each observation. They are much shorter for the difference between emales and
...
means.
* In right-most column of the table, it is easy to see that females have a
...
But if the lines cross, that will be more difficult to see.
```
...
Moderation refers to contexts that strengthen or diminish the effect of, for instance, a media campaign. Let us refer to this type of mediation as _effect strength moderation_. In our current example, we would hypothesize that the effect of George Clooney as an endorser is stronger for female participants than male participants.
...
* The main effect indicates that the endorser group means (bottom row of the
...
* For each effect worth interpretation, because it is sizable and/or statistically significant, report eta-squared (eta^2^) and interpret it in terms of effect size. If you have to calculate eta-squared by hand, divide the between-groups sum of squares of an effect by the total sum of squares (SPSS: corrected total).
...
All six subggroups are of nearly equal size and/or the Levene test on  
