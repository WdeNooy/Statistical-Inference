...
* 0,01 = small or weak effect; 1% variance explained matches a correlation r = 0,10 because R^2^ in simple regression analysis equals the correlation coefficient (r) squared,
...
```{r, eval=FALSE}
* The solid red arrows represent the difference between an individual score and
the average score of the group to which the individual belongs. For example,
the left-most orange dot represents the willigness score of a participant who was
exposed to Clooney as endorser. The orange line represents the average
willingness score of the participants who were exposed to Clooney. The dotted red
arrow is the difference between the individual's willingness score and the mean
score of it's group. The solid red arcs represent within-groups differences.
```
...
variance to obtain the overall standard devation.
```
...
Let us assume that we have done a study and have measured the willingness to donate of a sample of 12 participants in our study as depicted in Figure \@ref(fig:anova-between). Once we have our data, we first have a look at the percentage of variance that is explained, eta^2^. What does it mean if we say that a percentage of the variance is explained when we interpret eta^2^? 

The variance that we want to explain consists of the differences between the scores of the participants on the outcome variable and the overall or grand mean of all outcome scores. The dotted black arrows Figure \@ref(fig:anova-between) express the distances between outcome scores and the grand average. Squaring and averaging these distances over all observations gives us the total variance in outcome scores.

Once we know the overall explained variance, we have a closer look. The goal of our experiment is to explain why some of our participants have a willingness to donate that is far above the grand mean (black line in Figure \@ref(fig:anova-between)) while others score a lot lower. We know that we had hypothesized that participants will be influenced by whether they have seen Jolie or Clooney endorse the campaign or not (no endorser condition). If an endorser is more effective, the overall level of willingness should be higher. In other words, the average willingness should be higher for participants confronted with this endorser.
...
The assumption of equal population variances is less important if group samples are more or less of equal size (a balanced design, see Section \@ref(balanced)). We use a rule of thumb that groups are of equal size if the size of the largest group is less than 10% larger than the size of the smallest group. If this is the case, we do not care about the assumption of homogeneous population variances.
...
which groups score higher and which groups score lower in the populaiton. We
...
```{r eval=FALSE}
* Yes, this is possible in principle. But the samples in the figure are too
small to get that situation.
```

3. Is it okay that we apply both an F test and several t tests to the same group differences?

```{r eval=FALSE}
* This is okay provided that we correct for capitalization on chance.
```
...
The Levene test is not statistically significant, F (2, 82) = 0.52, p = .595,  
so we may assume that smoking attitude for the three groups have equal  
population variances.  
  
Interpret the results:  
  
In the sample, former smkers have a much more negative attitude twoards
smoking (M = -1.69, SD = 1.71) than non-smokers (M = 0.64, SD = 1.17) and
smokers (M = 0.80, SD = 1.67). However, average attitude towards smoking
scores are not significantly different, F (2, 82) = 1.04, p = .358. Therefore,
we do not have sufficient evidence to conclude that one group has a more
positive attitude towards smoking than another group in the population of
adults.
```
...
distant from the grand (or overall) mean.
...
endorser and sex. We will see the means of subgroups: females exposed to
...
NO moderation.
* In the example, the red and blue lines are parallel, so the effect of
endorser on willingness is not moderated by sex.
...
```{r eval=FALSE}
* An effect is stronger if the group averages vary more strongly.
* Change the means for the males such that there is more variation between the
three blue triangles (male average scores per endorser) than among the red
dots (female average scores per endorser).
```
...
```{r eval=FALSE}
* For effects to cancel out, the difference between females and males in
willingness if they are exposed to Clooney should have opposite signs. If
males score higher than females if they are exposed to Jolie, females should
score higher than men if they are exposed to Clooney.
* If you ensure that the average score for Clooney, Jolie, and participants not
exposed to a celebrity endorser are the same, there is no main effect of
endorser.
```
...
* Of course, if you start with a table with equal average scores for all
subgroup, you have a situation with no main effects.
...
We can use the between-groups and within-groups variances to execute an F test just like the F test we use for main effects. The effect of differences among subgroups on the outcome variable (if main effects have been corrected) is called an _interaction effect_. The null hypothesis of an F test on an interaction effect states that the subgroups have the same population averages if we correct for the main effects. In other words, the null hypothesis is that the effect of one factor is not moderated by the other factor. 
...
* The test result for every effect, consisting of the test name (_F_), the degrees of freedom, and the significance (p value). APA6 prescribes the following format if you report the test result within your text: F (df1, df1) = F value, p = p value. 
...
* Report it if the main assumption is violated, that is, if you have (sub)groups of unequal size and the test on homogeneous variances is statistically significant. Report Levene's test just like you report the F test of a main effect (see above). We report and interpret the results of the analysis of variance but we warn that the results may not be trustworthy. 
