...
#### Flow chart {-}
```{r choice-diagram-copy, echo=FALSE, screenshot.alt = "figures/choice-diagram.png"}
source("flowchart.R")
p
rm(p)
```
...
(ref:LeveneSPSS) Levene's F test on equal population variances in SPSS.

Figure \@ref(fig:SPSSLevene): (ref:LeveneSPSS)

(ref:TindepSPSS) An independent-samples t test in SPSS.

Figure \@ref(fig:SPSSTindep): (ref:TindepSPSS)

(ref:TpairedSPSS) A paired-samples t test in SPSS.

Figure \@ref(fig:SPSSTpaired): (ref:TpairedSPSS)

(ref:1waySPSS) One-way analysis of variance (ANOVA) in SPSS.

Figure \@ref(fig:SPSS1way): (ref:1waySPSS)

(ref:correlationSPSS) Correlations with SPSS. 

Figure \@ref(fig:SPSScorrelation): (ref:correlationSPSS)

(ref:regsimpleSPSS) Simple regression analysis in SPSS.

Figure \@ref(fig:SPSSregsimple): (ref:regsimpleSPSS)

(ref:chisqcrossSPSS) Chi-squared test on a contingency table with SPSS.

Figure \@ref(fig:SPSSchisqcross): (ref:chisqcrossSPSS)
...
(ref:reglines1SPSS) Adding a regression line to a scatterplot in SPSS.

Figure \@ref(fig:SPSSreglines1): (ref:reglines1SPSS)
